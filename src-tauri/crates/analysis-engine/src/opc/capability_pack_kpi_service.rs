//! 域包 KPI 计算服务 — 迁移自 OpcCapabilityPackAdapter::compute_kpis
//!
...
/// `crate::opc::capability_pack::runtime_schema`）。
/// 两侧唯一的契约是 `key`。
...
/// 内容媒体 KPI 计算 —— **按 key 查注册表**逐项取数（无 if-capability_pack 特例）。
///
...
pub async fn compute_kpis(
    ...
    match domain_pack_id.replace('-', "_").as_str() {
        ...
        _ => {
            let config = super::capability_pack_config::get_config(domain_pack_id, None);
...
mod tests {
    ...
    #[test]
    fn kpi_source_lookup_normalizes_domain_pack_id_and_isolates_capability_packs() {
        assert!(matches!(
