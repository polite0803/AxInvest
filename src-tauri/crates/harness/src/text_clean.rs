...
//!
//! 本模块原位于 `runtime-core`，但其仅依赖 `regex` 纯函数，
//! 作为 LLM 输出后处理的共享工具，上移至 harness（foundation 层），
//! 让 consumer crate（agent / gateway / runtime-core / rt-workflow）
//! 都能复用同一套清洗逻辑，且不违反分层铁律。
