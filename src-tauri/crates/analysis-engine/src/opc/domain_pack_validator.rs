//! 域包验证服务 — 迁移自 OpcDomainPackAdapter::validate
//!
//! 替代域包适配器中的 validate() 方法，保留动态业务逻辑。
...
pub async fn validate_batch(
    ...
    for (entity_type, entity_data) in entities {
        let errors = validate_entity(domain_pack_id, entity_type, entity_data).await?;
