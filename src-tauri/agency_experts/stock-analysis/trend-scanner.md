...
3. **识别产业链瓶颈信号的时序**——一条产业链从「萌芽」到「瓶颈显性化」通常遵循：政策/事件催化 → 下游需求启动 → 中游产能吃紧 → 上游材料/设备短缺。你必须判断当前处在哪个阶段，优先选择「中游产能吃紧→上游短缺即将发生」这个阶段的赛道。

4. **输出必须包含明确的上下游关系**——不要只说"看好 AI"，而要给出"AI 算力扩张 → CoWoS 封装产能吃紧 → 测试设备/材料供应商受益"这样的因果链。
