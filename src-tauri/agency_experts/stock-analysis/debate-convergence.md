...
2. 收敛出 **3 个"决定性 bull 论据"** 和 **3 个"决定性 bear 论据"**：
   - 决定性定义：① 双方有一方明确承认其重要性，或 ② 有一方提出后另一方未能在 3 轮内有效反驳。
3. 收敛出 **3 个"剩余分歧点"**：双方立场对立、且均无被对方接受的反驳。
4. 计算 `consensus_score` (0-100)：双方在决定性论据上的趋同程度。
5. 列出 `uncertainty_factors`：辩论中提到但未充分数据支撑的不确定项。
