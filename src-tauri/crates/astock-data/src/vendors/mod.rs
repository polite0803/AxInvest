...
pub trait StockVendor: Send + Sync {
    ...
    /// 按关键词搜索新闻的 as-of 通道（T2, 2026-09-26）。
    /// 默认转调 live 版本；只有申报 `NativeDateParam` 的源才会被路由层调用。
    ...
    /// 必须显式失败，由路由层降级并留痕 —— 已实现者见 `eastmoney` / `tencent` 的 override。
...
/// 用指数日 K 线合成 as-of 时点的指数行情（eastmoney / tencent 共用）。
///
/// `klines` 必须按日期升序且来自**已按截止日取数**的请求
/// （`end=as_of`，或拿全量后由调用侧裁）；至少两根才能算昨收与涨跌幅。
/// 点位取末根收盘，昨收取前一根收盘。
pub(crate) fn index_quote_from_klines(
    code: &str,
    name: &str,
    klines: &[crate::types::KLine],
) -> Option<crate::types::IndexQuote> {
    if klines.len() < 2 {
        return None;
    }
