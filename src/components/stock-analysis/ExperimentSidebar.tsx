...
import { parseRiskLevel } from "@/types";
import { useStockAnalysisStore, type ExperimentRecord } from "@/stores/feature/stockAnalysisStore";
...
function computeDecision(params: WhatIfParams): {
  decision: string; confidence: number; positionPct: number; riskLevel: string;
} {
  const consensusAdj = ((params.consensusScore - 50) / 100) * 10;
  const dqiAdj = ((params.dqiScore - 50) / 100) * 5;
  const riskAdj = params.overallRisk === "低" ? 5 : params.overallRisk === "高" ? -5 : params.overallRisk === "极高" ? -10 : 0;
  const catBonus = params.catalystLevel === "L3估值体系级" ? 12 : params.catalystLevel === "L2业绩拐点级" ? 6 : params.catalystLevel === "L1普通消息" ? 2 : 0;
  ...
  const decision = confidence >= 80 && pos >= 30 ? "buy" : confidence >= 60 ? "buy" : confidence >= 40 ? "hold" : pos < 10 ? "sell" : "hold";
...
export function ExperimentSidebar() {
  ...
  const originalDecision = useMemo(() => decision ? {
  ...
  // Keep ref in sync
  useEffect(() => { paramRef.current = params; }, [params]);
  ...
  const handleAccept = useCallback(() => {
    const record: ExperimentRecord = {
      ...
      decisionBefore: decision ? { action: decision.action, confidence: decision.confidence, positionPct: decision.positionPct, riskLevel: decision.riskLevel } : {},
      decisionAfter: { action: result.decision as any, confidence: result.confidence, positionPct: result.positionPct, riskLevel: parseRiskLevel(result.riskLevel) },
      accepted: true,
      createdAt: Date.now(),
    };
  ...
  return (
    ...
    >
      {/* Header */}
      <div style={{
        background: "var(--color-background-info)", padding: "10px 12px",
        color: "var(--color-text-info)", fontWeight: 500, fontSize: 13,
      }}>
        Experiment
      ...
      <div style={{ padding: "10px 12px" }}>
        ...
        <div style={{ marginBottom: 10 }}>
          ...
          <ParamSlider label="consensusScore" value={params.consensusScore} min={0} max={100} onChange={(v) => setParams((p) => ({ ...p, consensusScore: v }))} />
          <ParamSlider label="totalScore" value={params.totalScore} min={0} max={100} onChange={(v) => setParams((p) => ({ ...p, totalScore: v }))} />
          <ParamSlider label="dqiScore" value={params.dqiScore} min={0} max={100} onChange={(v) => setParams((p) => ({ ...p, dqiScore: v }))} />
          <ParamSelect label="overallRisk" value={params.overallRisk} options={["低", "中", "高", "极高"]} onChange={(v) => setParams((p) => ({ ...p, overallRisk: v }))} />
          <ParamSelect label="catalystLevel" value={params.catalystLevel} options={["无催化剂", "L1普通消息", "L2业绩拐点级", "L3估值体系级"]} onChange={(v) => setParams((p) => ({ ...p, catalystLevel: v }))} />
          <ParamSelect label="institutionalTrace" value={params.institutionalTrace} options={["无异常", "疑似建仓", "有建仓痕迹", "资金出逃"]} onChange={(v) => setParams((p) => ({ ...p, institutionalTrace: v }))} />
        </div>
        ...
        <div style={{ borderTop: "0.5px solid var(--color-border-tertiary)", paddingTop: 8, marginBottom: 10 }}>
          ...
          {showConfig && (
            <div>
              <ParamSlider label="scoring_trend" value={configOverrides.scoring_trend ?? 30} min={0} max={100} onChange={(v) => setConfigOverrides((p) => ({ ...p, scoring_trend: v }))} />
              <ParamSlider label="scoring_deviation" value={configOverrides.scoring_deviation ?? 20} min={0} max={100} onChange={(v) => setConfigOverrides((p) => ({ ...p, scoring_deviation: v }))} />
              <ParamSlider label="kelly_fraction" value={configOverrides.kelly_fraction ?? 0.5} min={0} max={1} step={0.05} onChange={(v) => setConfigOverrides((p) => ({ ...p, kelly_fraction: v }))} />
        ...
        <div style={{ borderTop: "0.5px solid var(--color-border-tertiary)", paddingTop: 8, marginBottom: 10 }}>
          ...
          <div style={{ border: "0.5px solid var(--color-border-success)", borderRadius: 6, padding: "8px 10px" }}>
            <div style={{ display: "flex", gap: 16 }}>
              ...
              <div>
                <div style={{ fontSize: 10, color: "var(--color-text-secondary)" }}>{hasChanges ? "Modified" : "Same"}</div>
                <div style={{ fontSize: 13, fontWeight: 500, color: hasChanges ? "var(--color-text-success)" : "var(--color-text-secondary)", marginTop: 2 }}>
    ...
    <div style={{ marginBottom: 6 }}>
      ...
      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
        ...
        <InputNumber
          ...
          size="small"
          min={min} max={max} step={step ?? 1}
          value={value} onChange={(v) => onChange(v ?? min)}
          controls={false}
        />
      </div>
    </div>
