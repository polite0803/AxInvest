...
pub async fn run_initialization(db: &impl ConnectionTrait) -> Result<(), DbErr> {
    ...
    for sql in &[
        ...
        "CREATE TABLE IF NOT EXISTS trades (\
            ...
            analysis_id TEXT, created_at INTEGER NOT NULL)",
    ...
    // --- 迁移追踪表（记录已应用的迁移版本，用于 P0-1 schema 演进可见性） ---
    db.execute_unprepared(
    ...
    for (id, desc) in &[
        ...
        ("r1_2026_06_10", "R1 复盘→进化闭环（strategy_performance/weight_history）"),
        ("r2_2026_06_10", "R2 组合监控（portfolio_metrics_daily/correlation_snapshot）"),
