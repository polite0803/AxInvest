...
// =========================================================================
// AI Chat Action v2.0 — 反思闭环基础设施
// 上游 LLM system_prompt 提示词定义了 5 种新 action_type:
//   - update_variable        (L1 闭环：参数调整)
//   - rollback_to_version    (一键回滚)
//   - update_input_mapping   (改 sub-workflow 输入)
//   - edit_asset_file        (L2/L3 闭环：改 .rhai / .md 资产)
//   - apply_diff_with_validation (聚合器，见 workflow_apply.rs)
//
// 这些命令必须同时改 commands/mod.rs (声明模块) + lib.rs (generate_handler! 注册)
// =========================================================================

/// Action 1: 修改 workflow_template.variables 中的指定变量。
/// 用于 L1 闭环（参数调整）：
///   - 业务反思 LLM 输出 `params_suggestion` → 用户勾选 → `update_variable` 落库。
/// 业务层（如 stock-analysis 决策引擎）需要重新读取 variables 才能生效。
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateWorkflowVariableInput {
    pub template_id: String,
    /// 变量名（不支持点路径，保持简单）
    pub name: String,
    /// 新值（任意 JSON 标量 / 对象 / 数组）
    pub value: serde_json::Value,
    /// 可选：人工填写的修改理由，便于 audit
    pub reason: Option<String>,
}

#[tauri::command]
pub async fn update_workflow_template_variable(
    state: State<'_, AppState>,
    input: UpdateWorkflowVariableInput,
) -> Result<UpdateVariableResult, String> {
    let db = state.harness.db();
    use axagent_core::entity::workflow_template;
    use sea_orm::{ActiveModelTrait, EntityTrait, Set};

    let row = workflow_template::Entity::find_by_id(&input.template_id)
        .one(db)
        .await
        .map_err(|e| e.to_string())?
        .ok_or_else(|| format!("模板 {} 不存在", input.template_id))?;

    let mut variables: Vec<axagent_harness::workflow_types::Variable> = row
        .variables
        .as_deref()
        .and_then(|v| serde_json::from_str(v).ok())
        .unwrap_or_default();
    ...
    for var in &mut variables {
        if var.name == input.name {
            old_value = Some(std::mem::replace(&mut var.value, input.value.clone()));
            found = true;
            break;
        }
    }
    if !found {
        // 变量不存在 → 当作新增（L1 闭环允许新增变量，业务侧自行校验 schema）
        variables.push(axagent_harness::workflow_types::Variable {
            name: input.name.clone(),
            var_type: infer_var_type(&input.value),
            value: input.value.clone(),
            description: input.reason.clone(),
            is_secret: false,
        });
    }
    ...
    let mut am: workflow_template::ActiveModel = row.into();
    am.variables = Set(Some(variables_json));
    am.version = Set(new_version);
    am.updated_at = Set(now);
    am.update(db).await.map_err(|e| e.to_string())?;
...
fn infer_var_type(v: &serde_json::Value) -> String {
    match v {
...
/// Action 2: 一键回滚到指定版本。
/// 数据来源：`workflow_template_versions` 表（每次 `update_workflow_template`
/// 会自动写入快照，见 update_workflow_template 行 137-167）。
#[tauri::command]
pub async fn rollback_workflow_template_to_version(
    state: State<'_, AppState>,
    template_id: String,
    target_version: i32,
    ...
    .await
    .map_err(|e| e.to_string())?
    .ok_or_else(|| format!("模板 {} 的版本 {} 不存在", template_id, target_version))?;

    let current = workflow_template::Entity::find_by_id(&template_id)
        .one(db)
        .await
        .map_err(|e| e.to_string())?
        .ok_or_else(|| format!("模板 {} 不存在", template_id))?;
    ...
    // 先把当前版本存为新快照（确保回滚操作也可被再次回滚）
    let current_snapshot = workflow_template_version::ActiveModel {
        id: Set(format!("{}_v{}", current.id, current.version)),
        template_id: Set(current.id.clone()),
        name: Set(current.name.clone()),
        description: Set(current.description.clone()),
        icon: Set(current.icon.clone()),
        tags: Set(current.tags.clone()),
        version: Set(current.version),
        is_preset: Set(current.is_preset),
        is_editable: Set(current.is_editable),
        is_public: Set(current.is_public),
        trigger_config: Set(current.trigger_config.clone()),
        nodes: Set(current.nodes.clone()),
        edges: Set(current.edges.clone()),
        input_schema: Set(current.input_schema.clone()),
        output_schema: Set(current.output_schema.clone()),
        variables: Set(current.variables.clone()),
        error_config: Set(current.error_config.clone()),
        created_at: Set(now),
    };
    let cur_snap_id = format!("{}_v{}", current.id, current.version);
    let cur_exists = workflow_template_version::Entity::find_by_id(&cur_snap_id)
        .one(db)
        .await
        .map_err(|e| e.to_string())?
        .is_some();
    ...
    let mut am: workflow_template::ActiveModel = current.into();
    am.name = Set(snapshot.name.clone());
    am.description = Set(snapshot.description.clone());
    am.icon = Set(snapshot.icon.clone());
    am.tags = Set(snapshot.tags.clone());
    am.trigger_config = Set(snapshot.trigger_config.clone());
    am.nodes = Set(snapshot.nodes.clone());
    am.edges = Set(snapshot.edges.clone());
    am.input_schema = Set(snapshot.input_schema.clone());
    am.output_schema = Set(snapshot.output_schema.clone());
    am.variables = Set(snapshot.variables.clone());
    am.error_config = Set(snapshot.error_config.clone());
    am.version = Set(new_version);
    am.updated_at = Set(now);
    am.update(db).await.map_err(|e| e.to_string())?;

    tracing::info!(
        "[rollback] {} 已回滚 v{} → v{}, 新版本 v{}",
        template_id,
        target_version,
        target_version,
        new_version
...
/// Action 3: 改 sub-workflow 节点的 input_mapping。
/// input_mapping 的语义：把上游节点的输出 / variables 映射到本节点的 inputs。
/// `mappings` 是覆盖式（整组替换），调用方需传完整 mapping。
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateInputMappingInput {
    pub node_id: String,
...
pub struct InputMappingEntry {
    /// 本节点的 input 名
    pub target: String,
    /// 上游来源：变量名（"variables.xxx"）/ 上游节点 ID（"nodeId.field"）
    pub source: String,
}

#[tauri::command]
pub async fn update_workflow_node_input_mapping(
    state: State<'_, AppState>,
    input: UpdateInputMappingInput,
) -> Result<UpdateInputMappingResult, String> {
    let db = state.harness.db();
    use axagent_core::entity::workflow_template;
    use sea_orm::{ActiveModelTrait, EntityTrait, Set};

    // node_id 形如 "templateId:nodeId"（前端传过来的复合 ID），先拆分
    let (template_id, node_id) = parse_composite_node_id(&input.node_id);
    ...
    for node in &mut nodes {
        if node.base_id() == node_id {
            // 任何带 inputs 的节点都可以改 input_mapping（agent / subWorkflow / tool）
            let json = serde_json::to_value(&node).map_err(|e| format!("序列化失败: {e}"))?;
            let mut json = match json {
                serde_json::Value::Object(m) => m,
                _ => return Err("节点不是 object".into()),
            };
            let new_mappings = serde_json::to_value(&input.mappings)
                .map_err(|e| format!("序列化 mappings 失败: {e}"))?;
    ...
    tracing::info!(
        "[template] {} 节点 {} input_mapping 已更新 ({} 条), 新版本 {}",
        template_id,
        node_id,
        input.mappings.len(),
        new_version
...
#[derive(Debug, serde::Serialize)]
#[serde(rename_all = "camelCase")]
pub struct UpdateInputMappingResult {
    pub template_id: String,
    pub node_id: String,
    pub new_version: i32,
...
/// 把 "tmplId:nodeId" 拆成 ("tmplId", "nodeId")；如果没有冒号则整体当作 node_id，
/// template_id 留空，调用方需自行保证。
fn parse_composite_node_id(composite: &str) -> (String, String) {
    if let Some(idx) = composite.find(':') {
        (composite[..idx].to_string(), composite[idx + 1..].to_string())
    } else {
        (String::new(), composite.to_string())
    }
}

/// Action 4: 改 .rhai / .md 等硬编码资产文件。
/// L2 闭环（公式调整） / L3 闭环（提示词调整）。
/// 资产路径相对工作目录：例如
///   "src-tauri/src/commands/portfolio-mgr.rhai"
///   "src-tauri/agency_experts/stock-analysis/reflection.md"
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct EditAssetFileInput {
    pub path: String,
    /// insert_after / replace / delete
    pub operation: String,
    /// 锚点行号（1-based）；insert_after 在此行后插入，replace 替换以此行开头的块
    pub anchor_line: i32,
    /// insert_after / replace 必填
...
pub async fn edit_workflow_asset_file(
    ...
    if !matches!(input.operation.as_str(), "insert_after" | "replace" | "delete") {
    ...
    let new_content = match input.operation.as_str() {
        "insert_after" => {
            ...
            for (i, line) in code.lines().enumerate() {
                out.insert(anchor + 1 + i, line.to_string());
            }
        ...
        "replace" => {
            ...
            for (i, line) in code_lines.iter().enumerate() {
                if anchor + i < out.len() {
                    out[anchor + i] = line.to_string();
                } else {
                    out.push(line.to_string());
                }
            }
            if replace_count > 0 && anchor + replace_count <= out.len() {
                out.drain(anchor + replace_count..anchor + replace_count.min(out.len() - anchor));
            }
            out.push(String::new());
            out.join("\n")
        },
    ...
    // 落盘前先在 .bak 留一份（仅本会话内有效；下次启动会被覆盖）
    let backup_path = full_path.with_extension("bak");
    let _ = std::fs::write(&backup_path, &original);

    std::fs::write(&full_path, &new_content).map_err(|e| format!("写入文件失败: {e}"))?;
