...
use axagent_analysis_engine::opc::{DomainPackLearningManager, OpcDomainPackAnalysisRound};
...
/// 域包操作配置
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DomainPackActionConfig {
    /// 操作唯一标识
    pub key: String,
    /// 操作标签
    pub label: String,
...
/// 域包工作流配置
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DomainPackWorkflowConfig {
    /// 工作流唯一标识
    pub id: String,
    /// 工作流名称
    pub name: String,
...
pub struct DomainPackConfig {
    ...
    pub description: String,
    /// 域包专属操作列表
    pub actions: Vec<DomainPackActionConfig>,
    /// 域包专属工作流列表
    pub workflows: Vec<DomainPackWorkflowConfig>,
}

// ── 9 个域包配置定义 ──────────────────────────────────────────────

fn ai_research_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn software_dev_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn finance_invest_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn sales_growth_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn content_media_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn consulting_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn accounting_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn ecommerce_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn education_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn design_config() -> DomainPackConfig {
    DomainPackConfig {
        id: "design".to_string(),
        name: "设计".to_string(),
        icon: "🎨".to_string(),
...
fn project_management_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn security_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn geospatial_config() -> DomainPackConfig {
    DomainPackConfig {
...
fn game_dev_config() -> DomainPackConfig {
    DomainPackConfig {
...
/// 获取所有 14 个域包配置
pub fn get_all_domain_pack_configs() -> Vec<DomainPackConfig> {
    vec![
        ai_research_config(),
        software_dev_config(),
...
/// 根据域包 ID 获取配置
pub fn get_domain_pack_config(domain_pack_id: &str) -> Option<DomainPackConfig> {
    get_all_domain_pack_configs().into_iter().find(|c| c.id == domain_pack_id)
}

/// 根据域包 ID 和操作 key 获取操作配置
pub fn get_action_config(domain_pack_id: &str, action_key: &str) -> Option<DomainPackActionConfig> {
    get_domain_pack_config(domain_pack_id)
...
/// 接线：opc_domain_pack_bridge 的 opc_execute_domain_pack_action 工具 handler 调用。
pub fn execute_domain_pack_action(
    domain_pack_id: &str,
    action_key: &str,
...
// 接线：opc_domain_pack_bridge 的 opc_create_domain_pack_workflow 工具 handler 调用
pub fn create_domain_pack_workflow(
    domain_pack_id: &str,
    workflow_id: &str,
...
pub fn get_workflow_config(
    domain_pack_id: &str,
    workflow_id: &str,
) -> Option<DomainPackWorkflowConfig> {
    get_domain_pack_config(domain_pack_id)
...
#[tauri::command]
pub async fn opc_get_domain_pack_config(
...
#[tauri::command]
pub async fn opc_get_domain_pack(domain_pack_id: String) -> Result<serde_json::Value, String> {
    let config = get_domain_pack_config(&domain_pack_id)
        .ok_or_else(|| format!("域包不存在: {}", domain_pack_id))?;
...
#[tauri::command]
pub async fn opc_build_domain_pack_prompt(
    domain_pack_id: String,
    action_key: String,
...
#[tauri::command]
pub async fn opc_list_domain_pack_actions(
    domain_pack_id: String,
) -> Result<serde_json::Value, String> {
    let config = get_domain_pack_config(&domain_pack_id)
        .ok_or_else(|| format!("域包不存在: {domain_pack_id}"))?;
...
#[tauri::command]
pub async fn opc_list_domain_pack_workflows(
    domain_pack_id: String,
) -> Result<serde_json::Value, String> {
    let config = get_domain_pack_config(&domain_pack_id)
        .ok_or_else(|| format!("域包不存在: {domain_pack_id}"))?;
...
pub async fn opc_execute_analysis(
    ...
    let declared = crate::commands::opc_domain_pack_runtime::declared_kpi_thresholds(
        Some(&app_state.app_data_dir),
        &domain_pack_id,
    );
    let round = OpcDomainPackAnalysisRound::with_declared_thresholds(
...
pub(crate) async fn run_template_via_engine(
    ...
    // `crate::commands::constants::domain_pack::INPUT_KEY` 这一权威常量，不做字面量复写。
    let mut hook_input = serde_json::Map::new();
    hook_input.insert(
        crate::commands::constants::domain_pack::INPUT_KEY.to_string(),
...
pub async fn opc_execute_workflow(
    ...
    use axagent_analysis_engine::opc::domain_pack_config as engine_config;
    ...
    let template_id = if let Some(ref wf_id) = workflow_id {
        ...
        let domain_pack_config = get_domain_pack_config(&domain_pack_id);
        if let Some(config) = &domain_pack_config {
    ...
    let domain_pack_engine_config = engine_config::get_config(&domain_pack_id_normalized)
        .ok_or_else(|| format!("域包配置不存在: {domain_pack_id}"))?;

    // 组合工具解析器
    let tool_resolver = |names: &[String]| -> Vec<axagent_harness::workflow_types::ToolDef> {
        use axagent_tools::tools::domain_pack_defs::{
    ...
    let template_data = axagent_analysis_engine::opc::workflow::generate_domain_pack_template_data(
        &domain_pack_id_normalized,
        &domain_pack_engine_config,
        Some(&tool_resolver),
    );
...
pub async fn opc_get_learning_metrics(
    ...
    let mut manager = DomainPackLearningManager::new();
    let engine = manager.get_or_create(&domain_pack_id).clone();
...
mod tests {
    ...
    #[test]
    fn test_get_all_domain_pack_configs() {
        let configs = get_all_domain_pack_configs();
        assert_eq!(configs.len(), 14, "应有 14 个域包配置");
    }

    #[test]
    fn test_get_domain_pack_config() {
        let config = get_domain_pack_config("ai-research").expect("AI 研究域包应存在");
    ...
    #[test]
    fn test_build_domain_pack_prompt_with_input() {
    ...
    #[test]
    fn test_unknown_domain_pack_returns_none() {
        assert!(get_domain_pack_config("nonexistent").is_none());
    }
...
fn learning_file_for_pack(pack_dir: &std::path::Path) -> String {
    use axagent_analysis_engine::opc::domain_pack::read_manifest;
...
fn domain_pack_learning_config_path(
    domain_pack_id: &str,
    ...
    if let Some(path) = app_dir {
        let pack_dir =
            path.join(axagent_analysis_engine::opc::domain_pack::DOMAIN_PACKS_DIR).join(&dir_id);
        let candidate = pack_dir.join(learning_file_for_pack(&pack_dir));
        if candidate.is_file() {
            tracing::debug!("[domain_pack-learning] found via app_dir: {}", candidate.display());
    ...
    if let Some(repo_root) = crate::commands::opc_workflows::find_repo_config_dir(
        axagent_analysis_engine::opc::domain_pack::DOMAIN_PACKS_DIR,
    ) {
        let pack_dir = repo_root.join(&dir_id);
        let fallback = pack_dir.join(learning_file_for_pack(&pack_dir));
        if fallback.is_file() {
            tracing::debug!("[domain_pack-learning] found via repo_root: {}", fallback.display());
            return Some(fallback);
        }
    }

    tracing::warn!(
        "[domain_pack-learning] learning.yaml not found for {domain_pack_id} (app_dir={:?})",
...
pub struct DomainPackLearningConfigView {
    pub version: u32,
...
impl DomainPackLearningConfigView {
    /// 创建默认的学习配置（当 learning.yaml 不存在时使用）
    fn default_for(domain_pack_id: &str) -> Self {
        let domain_pack_names: std::collections::HashMap<&str, &str> = [
            ("ai-research", "人工智能研究"),
            ("software-dev", "软件开发"),
            ("finance-invest", "金融投资"),
            ("sales-growth", "销售增长"),
        ...
        let domain_pack_name =
            domain_pack_names.get(domain_pack_id).unwrap_or(&domain_pack_id).to_string();
...
/// [`domain_pack_learning_config_path`]（域包内 learning.yaml 优先）。
pub fn get_domain_pack_learning_config(
    domain_pack_id: &str,
    app_dir: Option<&std::path::Path>,
) -> Option<DomainPackLearningConfigView> {
    let domain_pack_names: std::collections::HashMap<&str, &str> = [
        ("ai-research", "人工智能研究"),
        ("software-dev", "软件开发"),
        ("finance-invest", "金融投资"),
    ...
    let config_path = domain_pack_learning_config_path(domain_pack_id, app_dir)?;
    ...
    Some(DomainPackLearningConfigView {
        version,
        domain_pack_id: domain_pack_id.to_string(),
        domain_pack_name,
        reflection_enabled,
...
pub fn get_all_domain_pack_learning_configs(
    app_dir: Option<&std::path::Path>,
) -> Vec<DomainPackLearningConfigView> {
    let base = axagent_analysis_engine::opc::domain_pack::resolve_domain_packs_dir(app_dir);
    let mut out = Vec::new();
    if let Ok(rd) = std::fs::read_dir(&base) {
        for entry in rd.filter_map(Result::ok) {
            ...
            if let Some(cfg) = get_domain_pack_learning_config(&domain_pack_id, app_dir) {
                out.push(cfg);
            }
        }
    }
...
pub async fn opc_get_learning_config(
    ...
    let config = get_domain_pack_learning_config(&domain_pack_id, Some(&state.app_data_dir))
        .unwrap_or_else(|| {
            tracing::warn!(
                "[opc_get_learning_config] 域包学习配置不存在，返回默认配置: {domain_pack_id}"
            );
            DomainPackLearningConfigView::default_for(&domain_pack_id)
        });
    serde_json::to_value(config).map_err(|e| {
        String::from(crate::commands::error::ErrorResponse::from_error(
            e,
...
pub async fn opc_list_learning_configs(
    ...
    let configs = get_all_domain_pack_learning_configs(Some(&state.app_data_dir));
...
pub async fn opc_reflect_on_workflow(
    ...
    // 获取域包适配器
    let registry = state.learning.domain_pack_adapter_registry.lock().await;
    ...
    // 调用学习引擎
    let engine = &state.learning.domain_pack_learning_engine;
...
#[tauri::command]
pub async fn opc_trigger_domain_pack_learning(
    state: State<'_, AppState>,
    domain_pack_id: String,
    workflow_id: String,
    workflow_result: serde_json::Value,
    ...
    if rl_config.enabled {
        ...
        let full_rl_config =
            load_rl_config(&domain_pack_id, Some(&state.app_data_dir)).unwrap_or(rl_config);
...
/// [`get_domain_pack_learning_config`] 一致（域包内 learning.yaml 优先）。
...
pub async fn opc_get_rl_stats(
    ...
    let engine = &state.learning.domain_pack_learning_engine;
    let stats = match domain_pack_id.filter(|s| !s.trim().is_empty()) {
        Some(id) => engine.get_domain_pack_experience_stats(&id).await,
...
pub async fn opc_trigger_rl_optimization(
    ...
    let rl_config = load_rl_config(&domain_pack_id, Some(&state.app_data_dir))
        .ok_or_else(|| format!("域包 {} 的 RL 配置不存在", domain_pack_id))?;

    if !rl_config.enabled {
        return Err(format!("域包 {} 的强化学习未启用", domain_pack_id));
    }

    let engine = &state.learning.domain_pack_learning_engine;
    let result = engine.optimize_policy(&domain_pack_id, &rl_config).await.map_err(|e| {
        String::from(crate::commands::error::ErrorResponse::from_error(
            e,
            crate::commands::error::ErrorCategory::Unrecoverable,
        ))
