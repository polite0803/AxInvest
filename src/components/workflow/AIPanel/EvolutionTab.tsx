...
import { Badge, Button, Card, Collapse, Empty, Tag, Typography, theme } from "antd";
import {
  Activity,
  ChevronDown,
  Clock,
    ...
    <div style={{ height: "100%", overflowY: "auto", padding: "12px" }}>
      ...
      {nodes.length === 0 ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {nodes.map((node) => {
            ...
            const srDiff =
              latest?.metrics?.success_rate != null && prev?.metrics?.success_rate != null
                ? latest.metrics.success_rate - prev.metrics.success_rate
                : null;
            const latDiff =
              latest?.metrics?.avg_latency_ms != null && prev?.metrics?.avg_latency_ms != null
              ...
              >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {latest ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 11, color: token.colorTextSecondary }}>
                        ...
                        {srDiff != null && (
                          ...
                          >
                            ...
                            成功率 {srDiff >= 0 ? "+" : ""}{(srDiff * 100).toFixed(1)}%
                          </span>
                        )}
                        {latDiff != null && (
                          <span
                          ...
                          >
                            ...
                            延迟 {latDiff >= 0 ? "+" : ""}{latDiff.toFixed(0)}ms
                          </span>
                        )}
                      </div>
                    ) : (
      ...
      <div style={{ marginTop: 16 }}>
        <Collapse
          ghost
          size="small"
          expandIcon={({ isActive }) => <ChevronDown size={12} style={{ transform: isActive ? "rotate(180deg)" : undefined, transition: "0.2s" }} />}
          items={[
            {
              key: "ab-test",
              label: (
              ...
              children: abResults.length === 0 ? (
                <Empty description="暂无活跃的 A/B 测试" image={Empty.PRESENTED_IMAGE_SIMPLE} style={{ padding: "8px 0" }} />
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    ...
                    >
                      ...
                      <div style={{ display: "flex", gap: 16, fontSize: 11, color: token.colorTextSecondary }}>
                        ...
                        {Object.entries(res.metrics_a).map(([k, v]) => (
                          <span key={k}>
                            {k}: A={typeof v === "number" ? v.toFixed(2) : String(v)} / B={typeof res.metrics_b[k] === "number" ? (res.metrics_b[k] as number).toFixed(2) : String(res.metrics_b[k])}
                          </span>
                        ))}
                      </div>
                    </Card>
