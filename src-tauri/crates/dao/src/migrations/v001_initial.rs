...
pub async fn up(db: sea_orm::DatabaseConnection) -> Result<(), DbErr> {
    ...
    // ========================================================================
    // SECTION K: AxInvest — 股本分析 & 量化交易表
    // 原本在 ddl.rs SECTION J / L 中创建,但 v005/v006/v007/v008 的
    // ALTER TABLE 依赖这些表存在,所以必须放在 v001_initial 中,否则
    // 全新数据库在 run_migrations() 阶段就会报 "no such table" 错误。
    //
    // 这些 CREATE 语句原始来源是 ddl.rs,此处保留 base schema。
    // ddl.rs 仍会以 IF NOT EXISTS 重跑,对已有/新建数据库都是幂等。
    // 后续 migration(v005-v009) 负责 ALTER TABLE 加列。
    // ========================================================================
    ...
    // stock_analyses 时间旅行列(在 v005/v006 前补上,否则 v005 ALTER TABLE 与
    // ddl.rs 后续的 ALTER TABLE 冲突)。`let _ = ` 吞已有列错误。
    let alter_stock_analysis = &[
    ...
    for sql in alter_stock_analysis {
        let _ = db.execute_unprepared(sql).await;
    }

    // reco_picks 附加索引列(ddl.rs 原有 ALTER TABLE,早于 v007 pick_data)
    let _ = db
        .execute_unprepared("ALTER TABLE reco_picks ADD COLUMN strategy_weights_json TEXT")
        .await;

    // stock_reflections 附加列(ddl.rs 原有 ALTER TABLE,早于 v008 结构化列)
    let _ = db
        .execute_unprepared(
