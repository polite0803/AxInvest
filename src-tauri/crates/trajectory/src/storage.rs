...
use axagent_core::entity::{
    self, trajectories, trajectory_entities, trajectory_learned_patterns,
...
impl TrajectoryStorage {
    ...
    pub fn with_fts(db: Arc<DatabaseConnection>, fts_db: Arc<std::sync::RwLock<rusqlite::Connection>>) -> Self {
        let fts_searcher = FTS5Search::new(fts_db, FTS5Config::default());
        Self { db, fts_searcher: Some(fts_searcher) }
    }
    ...
    pub fn save_trajectory(&self, trajectory: &Trajectory) -> Result<()> {
        ...
        rt.block_on(async {
            let am = trajectories::ActiveModel {
                id: Set(trajectory.id.clone()),
                session_id: Set(trajectory.session_id.clone()),
                user_id: Set(trajectory.user_id.clone()),
                topic: Set(trajectory.topic.clone()),
                summary: Set(trajectory.summary.clone()),
                outcome: Set(format!("{:?}", trajectory.outcome).to_lowercase()),
                duration_ms: Set(trajectory.duration_ms as i64),
                quality_overall: Set(trajectory.quality.overall),
                quality_task_completion: Set(trajectory.quality.task_completion),
                quality_tool_efficiency: Set(trajectory.quality.tool_efficiency),
                quality_reasoning_quality: Set(trajectory.quality.reasoning_quality),
                quality_user_satisfaction: Set(trajectory.quality.user_satisfaction),
                value_score: Set(trajectory.value_score),
                patterns: Set(serde_json::to_string(&trajectory.patterns)?),
                created_at: Set(trajectory.created_at.to_rfc3339()),
                replay_count: Set(trajectory.replay_count as i32),
                last_replay_at: Set(trajectory.last_replay_at.map(|dt| dt.to_rfc3339())),
            };
            ...
            trajectory_steps::Entity::delete_many()
                .filter(trajectory_steps::Column::TrajectoryId.eq(&trajectory.id))
                .exec(self.db.as_ref()).await?;
            for (idx, step) in trajectory.steps.iter().enumerate() {
                trajectory_steps::ActiveModel {
                    trajectory_id: Set(trajectory.id.clone()),
            ...
            trajectory_rewards::Entity::delete_many()
                .filter(trajectory_rewards::Column::TrajectoryId.eq(&trajectory.id))
                .exec(self.db.as_ref()).await?;
            for reward in &trajectory.rewards {
                trajectory_rewards::ActiveModel {
                    id: Set(Uuid::new_v4().to_string()),
                    trajectory_id: Set(trajectory.id.clone()),
                    reward_type: Set(format!("{:?}", reward.reward_type)),
                    value: Set(reward.value),
                    created_at: Set(chrono::DateTime::from_timestamp_millis(reward.timestamp_ms as i64)
    ...
    pub fn get_trajectory(&self, id: &str) -> Result<Option<Trajectory>> {
        let rt = tokio::runtime::Handle::try_current()
            .unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let model = trajectories::Entity::find_by_id(id).one(self.db.as_ref()).await?;
            Ok(match model {
                Some(m) => {
                    let steps = self.get_trajectory_steps(&m.id)?;
                    let rewards = self.get_trajectory_rewards(&m.id)?;
                    Some(model_to_trajectory(&m, steps, rewards))
                }
                None => None,
            })
        })
    ...
    pub fn get_trajectories(&self, limit: Option<usize>) -> Result<Vec<Trajectory>> {
        let rt = tokio::runtime::Handle::try_current()
            .unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let mut query = trajectories::Entity::find()
                .order_by_desc(trajectories::Column::CreatedAt);
            if let Some(l) = limit {
                let models: Vec<_> = query.all(self.db.as_ref()).await?.into_iter().take(l).collect();
                // 需要重新查询，这里用 all + take
            }
            ...
            for m in models {
                let steps = self.get_trajectory_steps(&m.id)?;
                let rewards = self.get_trajectory_rewards(&m.id)?;
                result.push(model_to_trajectory(&m, steps, rewards));
            }
            Ok(result)
        })
    }
    ...
    fn get_trajectory_rewards(&self, trajectory_id: &str) -> Result<Vec<RewardSignal>> {
        let rt = tokio::runtime::Handle::try_current()
            .unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let models = trajectory_rewards::Entity::find()
                .filter(trajectory_rewards::Column::TrajectoryId.eq(trajectory_id))
                .all(self.db.as_ref()).await?;
            Ok(models.into_iter().map(|r| {
                let reward_type = match r.reward_type.as_str() {
                ...
                RewardSignal {
                    reward_type, value: r.value, step_index: 0,
    ...
    pub fn save_skill(&self, skill: &Skill) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let am = trajectory_skills::ActiveModel {
                ...
                avg_execution_time_ms: Set(skill.avg_execution_time_ms),
            };
            trajectory_skills::Entity::insert(am)
                .on_conflict(sea_orm::sea_query::OnConflict::column(trajectory_skills::Column::Id)
                    .update_columns([trajectory_skills::Column::Name, trajectory_skills::Column::Content,
                        ...
                        trajectory_skills::Column::SuccessRate, trajectory_skills::Column::AvgExecutionTimeMs])
                    .to_owned())
                .exec(self.db.as_ref()).await?;
            Ok(())
        })
    ...
    pub fn get_skills(&self) -> Result<Vec<Skill>> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let models = trajectory_skills::Entity::find()
                .order_by_desc(trajectory_skills::Column::UsageCount)
                .all(self.db.as_ref()).await?;
            Ok(models.iter().map(|s| model_to_skill(s)).collect())
        })
    }
    ...
    pub fn record_skill_execution(&self, skill_id: &str, trajectory_id: Option<&str>,
        success: bool, execution_time_ms: u64, input_args: Option<&serde_json::Value>,
        output_result: Option<&serde_json::Value>) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            trajectory_skill_executions::ActiveModel {
                id: Set(Uuid::new_v4().to_string()),
                skill_id: Set(skill_id.to_string()),
                trajectory_id: Set(trajectory_id.map(|s| s.to_string())),
                success: Set(success as i32),
                execution_time_ms: Set(execution_time_ms as i64),
                created_at: Set(Utc::now().to_rfc3339()),
                input_args: Set(input_args.map(|v| serde_json::to_string(v).unwrap_or_default())),
                output_result: Set(output_result.map(|v| serde_json::to_string(v).unwrap_or_default())),
            }.insert(self.db.as_ref()).await?;
            Ok(())
        })
    }
    ...
    pub fn get_skill_analytics(&self, skill_id: &str) -> Result<SkillAnalytics> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let total = trajectory_skill_executions::Entity::find()
                .filter(trajectory_skill_executions::Column::SkillId.eq(skill_id))
                .count(self.db.as_ref()).await?;
            let successes = trajectory_skill_executions::Entity::find()
                .filter(trajectory_skill_executions::Column::SkillId.eq(skill_id))
                .filter(trajectory_skill_executions::Column::Success.eq(1))
                .count(self.db.as_ref()).await?;
            let trend: Vec<bool> = trajectory_skill_executions::Entity::find()
                .filter(trajectory_skill_executions::Column::SkillId.eq(skill_id))
                .order_by_desc(trajectory_skill_executions::Column::CreatedAt)
                .all(.all(self.db.as_ref()).await?
                .into_iter().map(|e| e.success != 0).collect();
            Ok(SkillAnalytics {
                total_executions: total as u32,
                success_rate: if total > 0 { successes as f64 / total as f64 } else { 0.0 },
                avg_execution_time_ms: 0.0, recent_executions: trend.len() as u32,
            })
        })
    }
    ...
    pub fn save_pattern(&self, pattern: &TrajectoryPattern) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let am = trajectory_patterns::ActiveModel {
                id: Set(pattern.id.clone()),
                name: Set(pattern.name.clone()),
                description: Set(pattern.description.clone()),
                pattern_type: Set(pattern.pattern_type.clone()),
                trajectory_ids: Set(serde_json::to_string(&pattern.trajectory_ids)?),
                frequency: Set(pattern.frequency as i32),
                success_rate: Set(pattern.success_rate),
                average_quality: Set(pattern.average_quality),
                average_value_score: Set(pattern.average_value_score),
                reward_profile: Set(serde_json::to_string(&pattern.reward_profile)?),
                created_at: Set(pattern.created_at.to_rfc3339()),
            };
            trajectory_patterns::Entity::insert(am)
                .on_conflict(sea_orm::sea_query::OnConflict::column(trajectory_patterns::Column::Id)
                    .update_columns([trajectory_patterns::Column::Name, trajectory_patterns::Column::Frequency,
                        trajectory_patterns::Column::SuccessRate, trajectory_patterns::Column::AverageQuality])
                    .to_owned())
                .exec(self.db.as_ref()).await?;
            Ok(())
        })
    ...
    pub fn get_patterns(&self) -> Result<Vec<TrajectoryPattern>> {
        ...
        rt.block_on(async {
            let models = trajectory_patterns::Entity::find()
                ...
                .all(self.db.as_ref()).await?;
            Ok(models.iter().map(model_to_traj_pattern).collect())
        })
    }

    pub fn get_patterns_by_success_rate(&self, min_success_rate: f64, limit: Option<usize>) -> Result<Vec<TrajectoryPattern>> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        let l = limit.unwrap_or(100);
        rt.block_on(async {
            let models = trajectory_patterns::Entity::find()
                .filter(trajectory_patterns::Column::SuccessRate.gte(min_success_rate))
                .order_by_desc(trajectory_patterns::Column::SuccessRate)
                .all(self.db.as_ref()).await?;
            Ok(models.iter().take(l).map(model_to_traj_pattern).collect())
        })
    }
    ...
    pub fn save_entity(&self, entity: &Entity) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let am = trajectory_entities::ActiveModel {
                id: Set(entity.id.clone()),
                name: Set(entity.name.clone()),
                entity_type: Set(serde_json::to_string(&entity.entity_type).unwrap_or_default()),
                properties: Set(serde_json::to_string(&entity.properties).unwrap_or_else(|_| "{}".to_string())),
                aliases: Set(serde_json::to_string(&entity.aliases).unwrap_or_else(|_| "[]".to_string())),
                first_seen_at: Set(entity.first_seen_at.to_rfc3339()),
                last_seen_at: Set(entity.last_seen_at.to_rfc3339()),
                mention_count: Set(entity.mention_count),
                confidence: Set(entity.confidence),
                created_at: Set(entity.created_at.map(|dt| dt.to_rfc3339())),
                updated_at: Set(entity.updated_at.map(|dt| dt.to_rfc3339())),
            };
            trajectory_entities::Entity::insert(am)
                .on_conflict(sea_orm::sea_query::OnConflict::column(trajectory_entities::Column::Id)
                    .update_columns([trajectory_entities::Column::Name, trajectory_entities::Column::LastSeenAt,
                        trajectory_entities::Column::MentionCount, trajectory_entities::Column::Confidence])
                    .to_owned())
                .exec(self.db.as_ref()).await?;
            Ok(())
        })
    ...
    pub fn get_all_entities(&self) -> Result<Vec<Entity>> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let models = trajectory_entities::Entity::find()
                .order_by_desc(trajectory_entities::Column::LastSeenAt)
                .all(self.db.as_ref()).await?;
            Ok(models.iter().map(|e| model_to_entity(e)).collect())
        })
    }
    ...
    pub fn search_entities(&self, query: &str, limit: usize) -> Result<Vec<Entity>> {
        ...
        rt.block_on(async {
            ...
            let models = trajectory_entities::Entity::find()
                .filter(trajectory_entities::Column::Name.like(&pattern))
    ...
    pub fn delete_entity(&self, id: &str) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async { trajectory_entities::Entity::delete_by_id(id).exec(self.db.as_ref()).await?; Ok(()) })
    }
    ...
    pub fn save_relationship(&self, rel: &Relationship) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let am = trajectory_relationships::ActiveModel {
                id: Set(rel.id.clone()),
            ...
            trajectory_relationships::Entity::insert(am)
    ...
    pub fn get_relationships_by_entity(&self, entity_id: &str) -> Result<Vec<Relationship>> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let models = trajectory_relationships::Entity::find()
                .filter(trajectory_relationships::Column::SourceId.eq(entity_id)
                    .or(trajectory_relationships::Column::TargetId.eq(entity_id)))
                .all(self.db.as_ref()).await?;
            Ok(models.iter().map(|r| model_to_relationship(r)).collect())
        })
    }
    ...
    pub fn delete_relationship(&self, id: &str) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async { trajectory_relationships::Entity::delete_by_id(id).exec(self.db.as_ref()).await?; Ok(()) })
    }
    ...
    pub fn save_session(&self, session: &Session) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let am = trajectory_sessions::ActiveModel {
                id: Set(session.id.clone()), title: Set(session.title.clone()),
                platform: Set(session.platform.clone()), user_id: Set(session.user_id.clone()),
                model: Set(session.model.clone()), system_prompt: Set(session.system_prompt.clone()),
                ...
                token_input: Set(session.token_input), token_output: Set(session.token_output),
            };
            trajectory_sessions::Entity::insert(am)
    ...
    pub fn update_session(&self, id: &str, updates: &SessionUpdate) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let mut am: trajectory_sessions::ActiveModel = trajectory_sessions::Entity::find_by_id(id)
                .one(self.db.as_ref()).await?
                .ok_or_else(|| anyhow::anyhow!("Session not found"))?
                .into_active_model();
            if let Some(ref title) = updates.title { am.title = Set(title.clone()); }
    ...
    pub fn delete_session(&self, id: &str) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async { trajectory_sessions::Entity::delete_by_id(id).exec(self.db.as_ref()).await?; Ok(()) })
    }
    ...
    pub fn save_message(&self, message: &Message) -> Result<()> {
        ...
        rt.block_on(async {
            trajectory_messages::ActiveModel {
                id: Set(message.id.clone()), session_id: Set(message.session_id.clone()),
                role: Set(message.role.clone()), content: Set(message.content.clone()),
                tool_calls: Set(message.tool_calls.clone()), tool_results: Set(message.tool_results.clone()),
                usage: Set(message.usage.clone()), created_at: Set(message.created_at.to_rfc3339()),
            }.insert(self.db.as_ref()).await?;
            Ok(())
        })
    }
    ...
    pub fn get_messages_by_session(&self, session_id: &str) -> Result<Vec<Message>> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            Ok(trajectory_messages::Entity::find().filter(trajectory_messages::Column::SessionId.eq(session_id))
    ...
    pub fn search_messages(&self, query: &str, limit: usize) -> Result<Vec<Message>> {
        ...
        rt.block_on(async {
            let pattern = format!("%{}%", query);
            Ok(trajectory_messages::Entity::find().filter(trajectory_messages::Column::Content.like(&pattern))
    ...
    pub fn get_all_memories(&self) -> Result<Vec<crate::memory::MemoryEntry>> {
        ...
        rt.block_on(async {
            Ok(trajectory_memories::Entity::find().all(self.db.as_ref()).await?.into_iter().map(|m| {
                crate::memory::MemoryEntry { id: m.id, content: m.content, memory_type: m.memory_type, updated_at: m.updated_at }
            }).collect())
        })
    }
    ...
    pub fn save_memory(&self, memory: &crate::memory::MemoryEntry) -> Result<()> {
        ...
        rt.block_on(async {
            trajectory_memories::Entity::insert(trajectory_memories::ActiveModel {
                id: Set(memory.id.clone()), content: Set(memory.content.clone()),
                memory_type: Set(memory.memory_type.clone()), updated_at: Set(memory.updated_at.clone()),
    ...
    pub fn delete_memory(&self, id: &str) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async { trajectory_memories::Entity::delete_by_id(id).exec(self.db.as_ref()).await?; Ok(()) })
    }
    ...
    pub fn save_learning_pattern(&self, pattern: &Pattern) -> Result<()> {
        ...
        rt.block_on(async {
            trajectory_learned_patterns::Entity::insert(trajectory_learned_patterns::ActiveModel {
                id: Set(pattern.id.clone()), pattern: Set(pattern.pattern.clone()),
                pattern_type: Set(pattern.pattern_type.clone()),
                success: Set(pattern.success), failure: Set(pattern.failure),
                last_used: Set(pattern.last_used.to_rfc3339()),
                created_at: Set(pattern.created_at.to_rfc3339()),
                metadata: Set(pattern.metadata.clone()),
            }).on_conflict(sea_orm::sea_query::OnConflict::column(trajectory_learned_patterns::Column::Id)
                .update_columns([trajectory_learned_patterns::Column::Success, trajectory_learned_patterns::Column::Failure,
                    trajectory_learned_patterns::Column::LastUsed]).to_owned())
                .exec(self.db.as_ref()).await?;
            Ok(())
        })
    }
    ...
    pub fn update_pattern_stats(&self, id: &str, success_delta: i32, failure_delta: i32) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let model = trajectory_learned_patterns::Entity::find_by_id(id).one(self.db.as_ref()).await?;
            if let Some(m) = model {
                let mut am: trajectory_learned_patterns::ActiveModel = m.into_active_model();
                am.success = Set(am.success.unwrap() + success_delta);
                am.failure = Set(am.failure.unwrap() + failure_delta);
    ...
    pub fn update_preference_by_key(&self, key: &str, updates: &Preference) -> Result<()> {
        let rt = tokio::runtime::Handle::try_current().unwrap_or_else(|_| tokio::runtime::Runtime::new().unwrap().handle().clone());
        rt.block_on(async {
            let model = trajectory_preferences::Entity::find()
    ...
    pub fn get_trajectory_stats(&self) -> Result<TrajectoryStatistics> {
        ...
        let mut total_quality = 0.0; let mut total_value = 0.0; let mut successes = 0;
        for traj in &trajectories {
            total_quality += traj.quality.overall; total_value += traj.value_score;
            if traj.outcome == TrajectoryOutcome::Success { successes += 1; }
        }
        Ok(TrajectoryStatistics { total_trajectories: total, total_sessions: 0, total_patterns: 0,
            avg_quality: total_quality / total as f64, avg_value_score: total_value / total as f64,
            success_rate: successes as f64 / total as f64, recent_trajectories: total.min(10) })
    }
    ...
    pub fn export_trajectories(&self, options: &TrajectoryExportOptions) -> Result<Vec<RLTrainingEntry>> {
        let query = TrajectoryQuery { session_id: None, user_id: None, topic: None,
            min_quality: options.min_quality, min_value_score: options.min_value_score,
            outcome: options.outcome_filter, time_range: None, limit: options.limit };
        Ok(self.query_trajectories(&query)?.into_iter().map(|t| t.export_as_rl()).collect())
    }
    ...
    pub fn create_fts_tables(&self) -> Result<()> {
        if let Some(ref fts) = self.fts_searcher { fts.create_fts_tables() } else { Ok(()) }
    }
    ...
    pub fn index_trajectory_fts(&self, trajectory: &Trajectory) -> Result<()> {
        if let Some(ref fts) = self.fts_searcher { fts.index_trajectory(trajectory, &trajectory.session_id) } else { Ok(()) }
    }
    ...
    pub fn index_memory_fts(&self, id: &str, memory_type: &str, content: &str, entities: &[String]) -> Result<()> {
        if let Some(ref fts) = self.fts_searcher { fts.index_memory(id, memory_type, content, entities) } else { Ok(()) }
    }
    pub fn delete_memory_fts(&self, id: &str) -> Result<()> {
        if let Some(ref fts) = self.fts_searcher { let _ = fts; } Ok(())
    }
    pub fn optimize_fts(&self) -> Result<()> {
        if let Some(ref fts) = self.fts_searcher { fts.optimize() } else { Ok(()) }
    }
}
...
fn model_to_skill(s: &trajectory_skills::Model) -> Skill {
    Skill {
        ...
        quality_score: 0.0, success_rate: s.success_rate,
...
fn model_to_sess(s: &trajectory_sessions::Model) -> Session {
    Session {
        ...
        token_input: s.token_input, token_output: s.token_output,
    }
}
...
impl TrajectoryQueue {
            ...
            loop {
                tokio::select! {
                    Some(trajectory) = rx.recv() => {
                        ...
                        if batch.len() >= 32 { Self::flush_batch(&storage_clone, &mut batch).await; }
                    }
                    _ = flush_interval.tick() => {
                        if !batch.is_empty() { Self::flush_batch(&storage_clone, &mut batch).await; }
                    }
                }
            }
        });
    ...
    pub fn try_enqueue(&self, trajectory: Trajectory) -> bool { self.sender.try_send(trajectory).is_ok() }
...
pub struct SessionUpdate {
    pub title: Option<String>, pub token_input: Option<i64>, pub token_output: Option<i64>,
}
...
pub struct TrajectoryStatistics {
    ...
    pub avg_quality: f64, pub avg_value_score: f64, pub success_rate: f64,
