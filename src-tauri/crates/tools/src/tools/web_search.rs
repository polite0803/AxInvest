...
impl Tool for WebSearchTool {
    ...
    async fn validate(&self, input: &Value, ctx: &ToolContext) -> Result<(), ToolError> {
        input["query"].as_str().ok_or_else(|| ToolError::invalid_input_for("WebSearch", "缺少 query 参数"))?;
    ...
    async fn call(&self, input: Value, _ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let text = axagent_core::search::execute_search_text("ddg", None, "", query, 8, 10000).await;
