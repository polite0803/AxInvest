...
> **术语边界 —— 本仓有两套同名「铁律」，勿混读：**
> ① **本文件**（`AGENTS.md`）的「铁律」= **规范级**：禁区 1–13、Harness 分层约束、Git 禁用操作。
>    面向**人 + 所有 AI 工具**，**按文件名全量加载**，讲「**该怎么写**」。权威且稳定。
> ② `.workbuddy/memory/MEMORY-RULES.md` 的「**铁律 #N**」= **判据级**：踩坑实证 + 检法（编号至 #175+）。
>    面向**本地记忆机制**，**按需加载**（注入层触发词召回），讲「**怎么检出这个坑**」，持续增长。
>
> 判据：**「必须被非本 AI 在动手前看到」→ 进本文件；「检出/诊断某类缺陷的方法」→ 进判据层。**
> 不要把判据层搬进本文件（175 条 ≈ 61 KB，全量加载会挤占每次会话的上下文预算）；
> 也不要把规范只写在判据层（它在其它 AI 工具里读不到）。
...
docs/                        # 📝 文档。**除根目录白名单外，所有文档一律放这里**（见「文档放置」）
├── audits/                  # 审计报告快照：AUDIT-<主题>-<YYYY-MM-DD>.md
├── plans/                   # 计划 / 提案 / 修复清单：PLAN-<主题>.md、PROPOSAL-<主题>.md
└── marketing/               # 产品宣传 / 对外介绍：PROMO.md、<主题>.md
```

## 文档放置（强制）

**根目录只允许这些 `.md`**（封闭白名单，共 15 个文件）：
`README.md` + `README-<LANG>.md`（GitHub 首页渲染）、`AGENTS.md`、`CLAUDE.md`（AI 工具按文件名在根目录自动加载）、`CHANGELOG.md`（git-cliff 生成落点）、`CHECKS.md`（发版检查清单入口）。
**其余 `.md` 一律不得落在仓库根目录 —— 包括「活文档」。**

> **唯一判据**：这份文档**必须放在根目录才能被工具 / 人或约定发现**吗？
> 能证明「放到 `docs/` 下就没人找得到」⇒ 才允许进白名单；否则一律进 `docs/`，**与它是否还在演进无关**。
> （典型反面判据是「它是活文档所以放根目录」—— 错。`PROMO.md` 就是活文档，但它 2026-09-14 已移入 `docs/marketing/`。）

| 文档类型 | 去处 | 命名 |
|---|---|---|
| 审计 / 体检 / 对比 / 根因定位报告（时点快照） | `docs/audits/` | `AUDIT-<主题>-<YYYY-MM-DD>.md` |
| 计划 / 提案 / 修复清单 | `docs/plans/` | `PLAN-<主题>.md` / `PROPOSAL-<主题>.md` |
| 产品宣传 / 对外介绍 / 非规范类活文档 | `docs/marketing/` | `PROMO.md` / `<主题>.md` |

**跨文档引用用纯文件名**（反引号包裹，如 `` `AUDIT-duplication-2026-09-13.md` ``），**不要写相对路径** —— 目录调整时不会断链，靠文件名搜索即可定位。源码注释里引用报告同理（只写文件名 + 章节号）。

**⚠ 典型违规**：把审计 / 计划报告写到仓库根目录。本项目 2026-09-13 单日新增 10+ 个此类文件，一度把根目录 `.md` 堆到 32 个，淹没了真正的活文档。
...
### ⚠️ Rust 验证门禁：`cargo check` / `cargo test` 全绿 ≠ 合规

本仓的 lint 规则**只有 clippy 会执行**。`clippy.toml` 的 `disallowed-types` 列了 5 个
`std::sync` 锁类型，`Cargo.toml` 的 `[workspace.lints.clippy]` 设为 `deny` —— 但
`cargo check` 与 `cargo test` **完全不跑这条规则**，它们照样全绿。

三条硬约束：

1. **必须 `--all-targets`** —— 否则 **test / bench target 不被 lint**。新写的测试代码正是高发区
   （2026-09-14 实测：新增测试替身用 `std::sync::Mutex` 记录调用参数，`cargo check` + `cargo test`
...
**先 grep 既有先例照抄形态，不要为了过门禁改语义** —— 例如把 `std::sync::Mutex` 换成
`tokio::sync::Mutex` 属于改语义（测试替身并不需要异步锁语义，且会引出同步函数的连锁改造）。
...
7. **命令注册（两步，但第二步是编译器做的）**：新增 Tauri 命令只需做两件事 —— ① 在 `commands/mod.rs` 声明 `pub mod`；② 给函数标注 `#[tauri::command]`。**无需手工维护任何命令列表**：`src-tauri/build.rs` 会解析上述两点，自动生成 `src/register_commands.rs`（该文件头部写明 `Auto-generated by build.rs — DO NOT EDIT`，同时另写一份到 `OUT_DIR`），`src/lib.rs:260` 只负责调用 `register_all_commands!()`。
   - ⚠️ **不要手改 `src/register_commands.rs`** —— 它是构建产物，改动会被下一次构建静默覆盖（所谓「改完不生效」的典型生成物陷阱）。它被保留在 `src/` 下，是为了让 `scripts/check-contracts.mjs` 等契约脚本能读到注册清单。
   - ⚠️ 若报 `cannot find __cmd__xxx in <模块>`，真因通常在**生成器**而非生成物：`commands/mod.rs` 中对含 `#[tauri::command]` 的子模块**只能写 `pub use sub::*;` 或逐条列出，禁止 `pub use sub::{a, b};`**（`build.rs` 会把 `::{a,b}` 与 `::*` 混同，用父模块路径注册子模块全部命令）。
...
**`文件:行号` 里的路径必须「段对齐」**（2026-09-18 立；判据落在 `scripts/check-single-source-facts.mjs` 的 `abbrevHitsFor`）：

| 写法 | 例 | 门禁结果 | 结论 |
|---|---|---|---|
| 完整仓库相对路径 | `src-tauri/src/commands/opc_domain_pack_actions.rs:2067` | 可定位 | ✔ 永远可用（**最稳**；只有目录搬迁会失效） |
| 唯一后缀（去掉前缀目录，**但从 `/` 边界起截断**） | `commands/opc_domain_pack_actions.rs:2067` | 可定位（`locate()` ③ 后缀消歧） | ✔ **允许**，比全路径更抗目录搬迁 |
| 裸 basename 且仓内唯一 | `cache_snapshot.rs` | 可定位 | ⚠ 允许但**脆**：仓内出现同名第二份即**静默**退化为「未检查」⇒ 建议写上一档 |
| **段内截断 / 改词**（砍中间段、换同义词） | `actions.rs` 代 `opc_domain_pack_actions.rs`；`domain_pack_bridge.rs` 代 `opc_domain_pack_bridge.rs` | **BROKEN**（仓内零同名）⇒ 落「真待查」桶 | ✖ **禁止** |

**唯一判据（机械）**：截断必须落在 **`/` 边界**上；从**文件名中间**截断、或换一个同义词，一律不许。
实测反例：`"opc_domain_pack_actions.rs".endsWith("actions.rs")` 为 `true`，但 `locate()` 要求的是 `/actions.rs` ⇒ 永不命中。
代价实证（2026-09-18）：`docs/plans/PLAN-industry-to-domain-migration.md` 的 `actions.rs` 一族 **12 条**，占当时 `BROKEN（真待查）` 桶的 **12/12（100%）** —— 那个桶因此**指向不了任何真实风险**。
门禁现已把这类单独成 `ABBREV` 桶，并在 `--dump-doc-refs` 里**逐条点出它缩写的真实文件**（自证，不是自陈）。
