...
/**
 * DomainPackHub — 域包业务统一入口（与 InvestHub 结构一致）。
 *
 * 将单个域包的业务流程集成到一个页面内的 Tab 中，按业务逻辑排序。
 * 与 `DomainHubPage`（8+1 个能力域的聚合总览页）不是同一层次：本组件服务单个域包。
 *
 * URL 参数：
 *   - ?tab=xxx — 当前激活的业务流程 Tab
 */
export function DomainPackHub({
  domainPackId,
  config,
  domainTitle,
  domainIcon,
