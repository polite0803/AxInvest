...
//! 域包规则引擎（无运行时容器，命令直调 — 与股票业务同架构）
//!
//! 宏观要求：OPC 域包与股票业务**同架构**。股票业务 = 引擎 + 命令直调（无业务
//! 运行时容器）；本模块同理——OPC 域包 = 内建手写 adapter（`analysis-engine`）+ 命令直调，
//! **没有 opc-runtime / registry / 数据驱动 adapter 注册表**（已移除）。
//!
//! 本模块仅保留通用规则引擎的纯函数，供命令层对 `CapabilityPackAutomationRule` 做条件求值
//! 与动作分发：
...
fn eval_condition(cond: &AutomationCondition, ctx: &HashMap<String, serde_json::Value>) -> bool {
    match cond {
        ...
        AutomationCondition::Custom { expression } => {
            tracing::debug!(
                "[opc-domain-pack] 跳过自定义条件表达式（需 Rhai 支持）: {}",
                expression
            );
            false
        },
...
pub async fn execute_rule_actions(
    ...
    rule: &CapabilityPackAutomationRule,
    context: &RuleContext,
) -> OpcResult<()> {
    for action in &rule.actions {
        match action {
            AutomationAction::SendNotification { target, message } => {
                tracing::info!("[opc-domain-pack] 通知: target={}, message={}", target, message);
            },
            AutomationAction::UpdateStatus { status } => {
                tracing::info!(
                    "[opc-domain-pack] 状态更新: entity={}/{} -> status={}",
                    context.entity_type,
                    context.entity_id,
                    status
                );
            ...
            AutomationAction::UpdateField { field, value } => {
                tracing::info!(
                    "[opc-domain-pack] 字段更新: entity={}/{} , field={}, value={}",
                    context.entity_type,
                    context.entity_id,
                    field,
                    value
            ...
            AutomationAction::CreateRecord { entity_type, data } => {
                tracing::info!(
                    "[opc-domain-pack] 创建记录: entity_type={}, data={}",
                    entity_type,
                    data
                );
                if let Some(ds) = data_service {
                    ...
                    tracing::info!(
                        "[opc-domain-pack] 记录创建成功: id={}, entity_type={}",
                        new_id,
                        entity_type
                    );
                }
