...
//! 直接调用 `opc_capability_pack_actions` 模块中的函数。
//!
//! 合并纪律：本文件为 AxInvest 本地新增，上游无此文件 → 永不冲突。

use crate::commands::agent::command_bridge::TauriCommandToolDef;
use crate::commands::opc_capability_pack_actions::{
    create_capability_pack_workflow, execute_capability_pack_action, get_action_config,
    get_all_capability_pack_configs, get_all_capability_pack_learning_configs,
...
pub fn build_opc_capability_pack_tool_defs() -> Vec<TauriCommandToolDef> {
    vec![
        // ── 域包列表（只读） ──
        TauriCommandToolDef {
        ...
        // ── 域包配置（只读） ──
        TauriCommandToolDef {
            name: "opc_get_capability_pack_config",
        ...
        // ── 构建域包对话 prompt（只读） ──
        TauriCommandToolDef {
            name: "opc_build_capability_pack_prompt",
        ...
        // ── 列出域包所有操作（只读） ──
        TauriCommandToolDef {
            name: "opc_list_capability_pack_actions",
        ...
        // ── 列出域包所有工作流（只读） ──
        TauriCommandToolDef {
            name: "opc_list_capability_pack_workflows",
        ...
        // ── 执行域包操作（写操作） ──
        TauriCommandToolDef {
            name: "opc_execute_capability_pack_action",
        ...
        // ── 创建域包工作流（写操作） ──
        TauriCommandToolDef {
            name: "opc_create_capability_pack_workflow",
        ...
        // ── 触发域包学习闭环（自动模式） ──
        TauriCommandToolDef {
            name: "opc_trigger_capability_pack_learning",
...
pub fn build_opc_capability_pack_chat_tools() -> Vec<ChatTool> {
    build_opc_capability_pack_tool_defs()
        .into_iter()
        .map(|def| ChatTool {
...
///
/// handler 直接调用 `opc_capability_pack_actions` 中的纯函数，无需数据库连接。
/// `opc_render_ui` 例外 —— 需要 `AppHandle` 发射 UI 渲染事件。
pub fn build_opc_capability_pack_handlers<R: tauri::Runtime>(
    app_handle: AppHandle<R>,
) -> Vec<(String, SkillToolHandler)> {
    let mut handlers = Vec::new();

    for def in build_opc_capability_pack_tool_defs() {
        let handler = create_opc_capability_pack_handler(def.name, app_handle.clone());
...
/// 创建单个域包命令的 handler
fn create_opc_capability_pack_handler<R: tauri::Runtime>(
    command_name: &str,
    app_handle: AppHandle<R>,
) -> SkillToolHandler {
    ...
    Box::new(move |input: &str| {
        ...
        execute_opc_capability_pack_command(&name, &input_value, &app_handle)
    })
}
...
fn execute_opc_capability_pack_command<R: tauri::Runtime>(
    command_name: &str,
    input: &Value,
    app_handle: &AppHandle<R>,
    ...
    match tokio::runtime::Handle::try_current() {
        Ok(handle) => handle
            .block_on(async { dispatch_opc_capability_pack_command(&name, input, &app).await }),
        Err(_) => {
...
async fn dispatch_opc_capability_pack_command<R: tauri::Runtime>(
    command_name: &str,
    input: &Value,
    app_handle: &AppHandle<R>,
) -> Result<String, String> {
    debug!("Executing OPC capability_pack command: {}", command_name);
    ...
    match command_name {
        ...
        "opc_get_capability_pack_config" => {
            let domain_pack_id = input["domain_pack_id"]
                .as_str()
                .ok_or_else(|| "缺少 domain_pack_id 参数".to_string())?;
            let config = get_capability_pack_config(domain_pack_id)
        ...
        "opc_list_capability_pack_actions" => {
            let domain_pack_id = input["domain_pack_id"]
                .as_str()
                .ok_or_else(|| "缺少 domain_pack_id 参数".to_string())?;
            let config = get_capability_pack_config(domain_pack_id)
        ...
        "opc_list_capability_pack_workflows" => {
            let domain_pack_id = input["domain_pack_id"]
                .as_str()
                .ok_or_else(|| "缺少 domain_pack_id 参数".to_string())?;
            let config = get_capability_pack_config(domain_pack_id)
        ...
        "opc_execute_capability_pack_action" => {
            let domain_pack_id = input["domain_pack_id"]
                .as_str()
                .ok_or_else(|| "缺少 domain_pack_id 参数".to_string())?;
            let action_key =
            ...
            let result = execute_capability_pack_action(domain_pack_id, action_key, user_input)?;
            serde_json::to_string_pretty(&result).map_err(|e| {
                String::from(crate::commands::error::ErrorResponse::from_error(
                    e,
                    crate::commands::error::ErrorCategory::Unrecoverable,
        ...
        "opc_create_capability_pack_workflow" => {
            let domain_pack_id = input["domain_pack_id"]
                .as_str()
                .ok_or_else(|| "缺少 domain_pack_id 参数".to_string())?;
            let workflow_id =
            ...
            let result = create_capability_pack_workflow(domain_pack_id, workflow_id, custom_name)?;
            serde_json::to_string_pretty(&result).map_err(|e| {
                String::from(crate::commands::error::ErrorResponse::from_error(
                    e,
                    crate::commands::error::ErrorCategory::Unrecoverable,
        ...
        "opc_get_learning_config" => {
            ...
            let config =
                get_capability_pack_learning_config(domain_pack_id, Some(&app_state.app_data_dir))
        ...
        "opc_list_learning_configs" => {
            let app_state = app_handle.state::<crate::AppState>();
            let configs = get_all_capability_pack_learning_configs(Some(&app_state.app_data_dir));
        ...
        "opc_reflect_on_workflow" => {
            ...
            let registry = app_state.learning.capability_pack_adapter_registry.lock().await;
            ...
            let engine = &app_state.learning.capability_pack_learning_engine;
        ...
        "opc_trigger_capability_pack_learning" => {
            let domain_pack_id = input["domain_pack_id"]
                .as_str()
                .ok_or_else(|| "缺少 domain_pack_id 参数".to_string())?;
            let workflow_id =
        ...
        other => {
            warn!("Unknown OPC capability_pack command: {}", other);
