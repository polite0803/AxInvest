...
import type { DomainConfig, DomainTab } from "./types";
  ...
  const handleExecute = async (workflowId: string) => {
    try {
      ...
      if (result.status === "completed") {
        message.success(t("opc.domain.tab.executeSuccess", { id: workflowId }));
      } else {
        message.error(t("opc.domain.tab.executeFailed", { id: workflowId }));
      }
    } catch {
      message.error(t("opc.domain.tab.executeFailed", { id: workflowId }));
    }
    ...
    <div style={{ padding: "16px 0" }}>
      ...
      {tab.actions.length > 0 && (
        <Card
          title={
            <span>
              <strong>{t("opc.domain.tab.actions")}</strong>
            </span>
          }
          style={{ marginBottom: 16 }}
          size="small"
              ...
              >
                ...
                <Tag color={action.type === "workflow" ? "blue" : "green"} style={{ fontSize: 11 }}>
                  {action.type === "workflow"
                    ? t("opc.domain.tab.type.workflow")
                    : t("opc.domain.tab.type.conversation")}
                </Tag>
              </div>
            ))}
          </div>
      ...
      {tab.workflows.length > 0 && (
        <Card
          title={
            <span>
              <strong>{t("opc.domain.tab.workflows")}</strong>
      ...
      {tab.actions.length === 0 && tab.workflows.length === 0 && (
        <Empty
          description={t("opc.domain.tab.noContent")}
        />
      )}
    </div>
  );
  ...
  return (
    <div style={{ padding: 24, height: "100%", overflow: "auto" }}>
      ...
      {data.workflowSteps.length > 0 && (
        <Card title={t("opc.domain.steps.title")} style={{ marginBottom: 24 }}>
          <Text type="secondary">{t("opc.domain.steps.total", { count: data.workflowSteps.length })}</Text>
        </Card>
      )}
      ...
      {data.workflowResult && (
        <Card
          title={t("opc.domain.tab.executionResult")}
          style={{ marginBottom: 24 }}
        >
          <div style={{ display: "flex", gap: 24, alignItems: "center" }}>
            ...
            <Text>
              {t("opc.domain.tab.durationMs", { ms: data.workflowResult.duration_ms })}
            </Text>
          </div>
        </Card>
      )}
