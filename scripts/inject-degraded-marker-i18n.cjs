#!/usr/bin/env node
/**
 * 注入 timeTravel.degradedMarker i18n key 到 11 个 locale 文件。
 * 在 pageAnchor 段后插入 degradedMarker 段。
 */
const fs = require("fs");
...
const TRANSLATIONS = {
  ...
  "en-US": {
    ...
    tooltip:
      "In replay mode, money flow / margin / north-bound holding etc. have no historical semantics and were skipped. See degraded hint in recommendation panel.",
  },
  "ja": {
    label: "降格 · 一部データに履歴なし",
    tooltip:
      "再生モードでは資金フロー/融資融券/北向保有などに履歴がなくスキップされました。詳細は推奨パネルの降格ヒントを参照。",
  },
  "ko": {
    label: "성능 저하 · 일부 데이터에 기록 없음",
    tooltip: "재생 모드에서 자금 흐름/마진/북향 보유 등에 기록이 없어 건너뛰었습니다. 추천 패널의 성능 저하 힌트 참조.",
  },
  "fr": {
...
for (const f of files) {
  ...
  // 路径: timeTravel.pageAnchor 段后插入 degradedMarker
  if (!data.timeTravel) {
    console.error(`[fail] ${f}: missing timeTravel root`);
    fail++;
    continue;
  }
  if (data.timeTravel.pageAnchor == null) {
    console.error(`[fail] ${f}: missing timeTravel.pageAnchor`);
    fail++;
    continue;
  }
  if (data.timeTravel.degradedMarker) {
    console.log(`[ok] ${f}: degradedMarker already present, skipping`);
    ok++;
    continue;
  }
  ...
  console.log(`[ok] ${f}: injected degradedMarker`);
  ok++;
}
