...
export function DomainTabContent({ domainPackId, config, tabKey }: DomainTabContentProps) {
  ...
  const [activeWorkflow, setActiveWorkflow] = useState<DomainWorkflow | null>(null);
  ...
  const handleStartWorkflow = (wf: DomainWorkflow) => {
    setActiveWorkflow(wf);
    setWizardOpen(true);
  };
  ...
  return (
    <div style={{ padding: "16px 24px", height: "100%", overflow: "auto" }}>
      ...
      {(!tab.actions || tab.actions.length === 0)
        && (!tab.workflows || tab.workflows.length === 0) && <Empty description={t("opc.domain.tab.noContent")} />}
