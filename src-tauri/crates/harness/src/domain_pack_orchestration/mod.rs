...
pub use types::{
    ...
    DependencyType, DomainPackContext, DomainPackLearningConfig, EvolutionConfig,
    EvolutionConstraints, ForbiddenOptimization, MissionType, PresetWorkflowStep, ProtectedStep,
    QualityThresholds, QualityWeights, ReflectionCheckpoint, ReflectionConfig, ReflectionTemplate,
...
/// - 验收标准定义
#[async_trait]
pub trait DomainPackAdapter: Send + Sync {
    /// 域包唯一标识
...
// ── DomainPackAdapterRegistry ────────────────────────────────────────

/// 域包适配器注册表
///
/// 管理所有域包适配器的实例，提供按 ID 查找功能。
pub struct DomainPackAdapterRegistry {
    adapters: Vec<Arc<dyn DomainPackAdapter>>,
}

impl DomainPackAdapterRegistry {
    /// 创建空注册表
    pub fn new() -> Self {
    ...
    /// 注册域包适配器
    pub fn register(&mut self, adapter: Arc<dyn DomainPackAdapter>) {
        self.adapters.push(adapter);
    }

    /// 按域包 ID 查找适配器
    pub fn get(&self, domain_pack_id: &str) -> Option<&Arc<dyn DomainPackAdapter>> {
    ...
    /// 获取所有已注册域包适配器引用
    pub fn all(&self) -> &[Arc<dyn DomainPackAdapter>] {
        &self.adapters
    }
...
impl Default for DomainPackAdapterRegistry {
    fn default() -> Self {
        Self::new()
    }
}
