...
//!
//! 启动扫描注册到 `opc_domain_packs` 表，支持单独启用/禁用/导出/导入。
...
/// 域包根目录（相对仓库根）
pub const DOMAIN_PACKS_DIR: &str = "config/opc/domain_packs";

/// **旧**域包根目录（2026-09-15 更名前）。
///
/// 仅用于 [`resolve_domain_packs_dir`] 的**存量回退读取**：
/// 升级前安装到 app_dir 的 `.opcip` 包仍在旧路径下，不设回退会静默读不到。
/// ⚠ 不要在任何「写」路径上使用本常量 —— 写入一律走 [`DOMAIN_PACKS_DIR`]。
///
/// ⚠ **刻意写成 `concat!` 拼接**：本仓的批量更名脚本按「字符串字面量 `config/opc/industries`」
/// 做替换，若此处直接写完整字面量，脚本会把**回退目标**一并改成新目录，
/// 使回退静默退化为「指向同一个目录」（实测已发生过一次）。
pub const LEGACY_DOMAIN_PACKS_DIR: &str = concat!("config/opc/", "industries");
...
pub struct DomainPackManifest {
    ...
    /// 供 `analysis_schema::load_domain_pack_analysis` 读取（domain_pack.rs:286）
    #[serde(default = "default_analysis_file")]
    pub analysis: String,
    /// 学习配置文件（P0-4 四件套之一），缺省 "learning.yaml"；读取见
    /// `opc_domain_pack_actions::domain_pack_learning_config_path`
    /// （`domain_pack_learning_config_path` 经 `read_manifest` 消费本字段决定实际文件名）
    #[serde(default = "default_learning_file")]
    pub learning: String,
    /// 运行时配置文件（P0-4 四件套之一），缺省 "runtime.yaml"。
    /// KPI **元数据**（key/name/unit/metric_type）与展示卡片的权威来源，
    /// 读取见 `runtime_schema::load_domain_pack_runtime`。
...
#[derive(Debug, Clone, Deserialize)]
pub struct DomainPackCapabilityClaim {
...
pub fn read_manifest(domain_pack_dir: &Path) -> Option<DomainPackManifest> {
    let raw = std::fs::read_to_string(domain_pack_dir.join("manifest.yaml")).ok()?;
    serde_yaml::from_str::<DomainPackManifest>(&raw).ok()
}

/// 扫描域包目录，返回所有 manifest（含是否启用）。
pub fn scan_domain_packs(base_dir: &Path) -> Vec<DomainPackManifest> {
    let mut out = Vec::new();
    ...
    for entry in entries.flatten() {
        ...
        match serde_yaml::from_str::<DomainPackManifest>(&raw) {
            Ok(m) => {
                out.push(m);
            },
            Err(e) => {
...
pub mod analysis_schema {
    ...
    use super::DomainPackManifest;

    /// 域包分析配置（`analysis.yaml`，由 manifest.analysis 字段引用，缺省同名文件）。
    ///
    /// 供数据接入层（OpDomainPackVendor 路由）、分析层（策略维度）与
    ...
    #[allow(dead_code)]
    pub struct DomainPackAnalysisConfig {
        #[serde(default)]
        pub version: u32,
        #[serde(default)]
    ...
    pub fn load_domain_pack_analysis(
        domain_pack_dir: &Path,
        manifest: &DomainPackManifest,
    ) -> Option<DomainPackAnalysisConfig> {
        let path = domain_pack_dir.join(&manifest.analysis);
    ...
    /// analysis/pack_dir 当前仅 load_domain_pack 装配，由 P1 数据接入层完整消费后移除豁免。
    #[derive(Debug, Clone)]
    #[allow(dead_code)]
    pub struct DomainPackBundle {
        pub manifest: DomainPackManifest,
        pub analysis: Option<DomainPackAnalysisConfig>,
        ...
        /// 学习配置在 `{domain_pack_dir}/{manifest.learning}`（P4-3 已迁入域包），
        /// 此处不重复解析，读取走 `opc_domain_pack_actions::domain_pack_learning_config_path`
        pub pack_dir: PathBuf,
    }

    /// 加载单个域包目录的完整资产（manifest 解析失败返回 None）。
    pub fn load_domain_pack(dir: &Path) -> Option<DomainPackBundle> {
        let manifest_path = dir.join("manifest.yaml");
        let raw = std::fs::read_to_string(&manifest_path).ok()?;
        let manifest: DomainPackManifest = serde_yaml::from_str(&raw).ok()?;
        let analysis = load_domain_pack_analysis(dir, &manifest);
        let runtime = super::runtime_schema::load_domain_pack_runtime(dir, &manifest);
        Some(DomainPackBundle { manifest, analysis, runtime, pack_dir: dir.to_path_buf() })
    }
}
...
///   `analysis_engine::opc::domain_pack_kpi_service`。
/// - 契约只有一个：`key`。
///
...
pub mod runtime_schema {
    ...
    pub struct RuntimeKpiDefinition {
        ...
        /// 由命令层经 `opc_domain_pack_runtime::declared_kpi_thresholds` 注入。
        #[serde(default)]
    ...
    /// runtime.yaml 的 KPI 元数据子集。
    ///
    /// **只解析 `kpi_definitions` + `dashboard_cards`（外加标识用的 `domain_pack_id`）
    /// 三处**；其余段（`validations` / `kpi_sources` / `automation_rules` /
    /// `workflow_steps`）由 serde 忽略。
    #[derive(Debug, Clone, Default, Deserialize)]
    ...
    /// 可观测），返回 `None` 由调用方兜底。
    pub fn load_domain_pack_runtime(
        domain_pack_dir: &Path,
        manifest: &DomainPackManifest,
    ) -> Option<RuntimeKpiConfig> {
        let path = domain_pack_dir.join(&manifest.runtime);
...
/// 将域包注册进 opc_domain_packs 表（存在则按 version 判断是否升级）。
pub async fn upsert_domain_pack_registry(
    ...
    let existing = opc_domain_packs::Entity::find_by_id(&m.id).one(db).await.ok().flatten();
    let effective_enabled = existing.map(|e| e.enabled != 0).unwrap_or(m.enabled);
    let am = opc_domain_packs::ActiveModel {
        id: Set(m.id.clone()),
        ...
        pack_path: Set(format!("{DOMAIN_PACKS_DIR}/{}", m.id)),
        installed_at: Set(now),
        updated_at: Set(now),
    };
    opc_domain_packs::Entity::insert(am)
        .on_conflict(
            sea_query::OnConflict::column(opc_domain_packs::Column::Id)
                .update_column(opc_domain_packs::Column::Name)
                .update_column(opc_domain_packs::Column::Icon)
                .update_column(opc_domain_packs::Column::Description)
                .update_column(opc_domain_packs::Column::Version)
                .update_column(opc_domain_packs::Column::Enabled)
                .update_column(opc_domain_packs::Column::PackPath)
                .update_column(opc_domain_packs::Column::UpdatedAt)
                .to_owned(),
        )
        .exec_without_returning(db)
        .await
        .map_err(|e| format!("upsert domain_pack: {e}"))?;
    Ok(())
}

/// 设置域包启用/停用状态 —— 写入 DB `opc_domain_packs.enabled`。
///
/// 权威存储是 DB（P1-5：manifest.enabled 仅首装生效，seed 后用户手动状态以 DB 为准），
/// 因此开关落盘这里，而不是改 manifest.yaml。
pub async fn set_domain_pack_enabled(
    db: &DatabaseConnection,
    id: &str,
    enabled: bool,
) -> Result<(), String> {
    ...
    opc_domain_packs::Entity::update(am)
        .exec(db)
        .await
        .map_err(|e| format!("set domain_pack enabled(id={id}): {e}"))?;
    Ok(())
}
...
/// 本函数仅负责 manifest 注册（opc_domain_packs 表），不再从 YAML 加载工作流。
///
/// 返回 seed 的域包 id 列表。
pub async fn ensure_opc_domain_packs_seeded(
    db: &DatabaseConnection,
    base_dir: &Path,
    ...
    let manifests = scan_domain_packs(base_dir);
    ...
    for m in manifests {
        ...
        upsert_domain_pack_registry(db, &m).await?;
        ...
        // 域包工作流已由手动定义的 seed 文件生成（seed_opc_domain_packs_from_seed_files），
        // 此处仅注册 manifest 到 opc_domain_packs 表。
        tracing::info!(
...
pub fn domain_pack_dir(base_dir: &Path, id: &str) -> PathBuf {
    base_dir.join(id)
}
...
/// 导出域包为 .opcip 归档。
/// 返回生成的文件路径。
pub async fn export_domain_pack(
    base_dir: &Path,
    id: &str,
    out_dir: &Path,
) -> Result<String, String> {
    let src = domain_pack_dir(base_dir, id);
    if !src.is_dir() {
...
/// 返回导入的域包 id。
pub async fn import_domain_pack(
    db: &DatabaseConnection,
    app_dir: &Path,
    archive_path: &Path,
) -> Result<String, String> {
    ...
    if archive_path.is_dir() {
        ...
        let industries_root = app_dir.join(DOMAIN_PACKS_DIR);
        ...
        let seeded = ensure_opc_domain_packs_seeded(db, &industries_root).await?;
    ...
    let target_root = app_dir.join(DOMAIN_PACKS_DIR);
    ...
    let seeded = ensure_opc_domain_packs_seeded(db, &target_root).await?;
    if !seeded.contains(&id) {
...
/// 域包根目录（相对仓库根，由调用方拼接）
pub fn domain_packs_base_dir() -> std::path::PathBuf {
    std::path::PathBuf::from(DOMAIN_PACKS_DIR)
}

/// 域包目录解析：app_dir/DOMAIN_PACKS_DIR → 旧目录 → 仓库根 fallback。
///
/// # 存量兼容（2026-09-15 目录更名）
///
/// 包根目录已由 [`LEGACY_DOMAIN_PACKS_DIR`] 更名为 [`DOMAIN_PACKS_DIR`]。
...
pub fn resolve_domain_packs_dir(app_dir: Option<&std::path::Path>) -> std::path::PathBuf {
    if let Some(dir) = app_dir {
        let candidate = dir.join(DOMAIN_PACKS_DIR);
        if candidate.is_dir() {
            return candidate;
        }
        let legacy = dir.join(LEGACY_DOMAIN_PACKS_DIR);
        if legacy.is_dir() {
            tracing::warn!(
...
// 本文件为权威 schema 所在（DomainPackManifest / DomainPackCapabilityClaim /
...
pub struct DomainPackCapabilityReport {
    /// 被校验的域包 id
    pub domain_pack_id: String,
    /// 域包声明的本体域归属
    pub domain: Option<CapabilityDomain>,
...
///
/// wiring 层（索引重建 `register_all_capabilities` / `opc_import_domain_pack` /
/// `ensure_opc_workflows_seeded` 三处）在护照收集完成后调用，共用本函数避免重复接线。
/// 对账口径与 `assert_domain_pack_capability_closed` 一致：工具共享、不背单指针，
/// 故实际护照取「全局工具护照 ∪ 反指本域包的动态（workflow/skill/agent）护照」。
/// 校验为**审计性质**（非硬失败）：报告用于日志/前端提示，不阻断任何流程。
pub async fn audit_domain_packs_capability<I: axagent_harness::CapabilityIndexer>(
    indexer: &I,
    base_dir: &std::path::Path,
) -> Vec<DomainPackCapabilityReport> {
    ...
    scan_domain_packs(base_dir)
        ...
        .map(|manifest| {
            let actual: Vec<CapabilityPassportDto> = all
                ...
                .cloned()
                .collect();
            assert_domain_pack_capability_closed(&manifest, &actual, &user_domains)
        })
        .collect()
}
...
mod tests {
    ...
    /// 定位仓库根的域包目录。
    ///
    /// 优先用生产装配路径 `resolve_domain_packs_dir(None)`；当它以相对路径行不被命中
    ...
    fn domain_packs_base_for_test() -> PathBuf {
        let resolved = super::resolve_domain_packs_dir(None);
        if resolved.is_dir() {
            return resolved;
        }
        ...
        loop {
            let candidate = dir.join(super::DOMAIN_PACKS_DIR);
            if candidate.is_dir() {
                return candidate;
            }
            match dir.parent() {
        ...
        panic!("找不到 {}（仓库根 config/opc/domain_packs）", super::DOMAIN_PACKS_DIR)
    }

    /// 验收：14 个内置域包 manifest 的 `capabilities` 承诺能正确从 YAML 反序列化，
    /// 且每条 predicate 都是 `IdExact`（本轮改动的关键验收点）。
    #[test]
    fn scan_builtin_domain_packs_parse_capabilities() {
        let base = domain_packs_base_for_test();
    ...
    fn assert_closed_when_claim_satisfied() {
        let manifest = DomainPackManifest {
            ...
            capabilities: vec![DomainPackCapabilityClaim {
        ...
        let report = assert_domain_pack_capability_closed(&manifest, &actual, &user_domains);
