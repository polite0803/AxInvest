 ...
 *
 * 核心设计：
 * - 步骤数组由 DomainWorkflow.wizardSteps 配置驱动
...
import type { DomainWorkflow, WizardContext, WizardStep, WorkflowInputField } from "./types";
import type { useDomainData } from "./useDomainData";

const { Text, Title } = Typography;

type DomainDataHook = ReturnType<typeof useDomainData>;
...
function createDefaultSteps(workflow: DomainWorkflow): WizardStep[] {
  ...
  if (hasInputs) {
    steps.push({
      id: "config",
      title: "opc.domain.wizard.steps.config",
      description: "opc.domain.wizard.steps.configDesc",
      type: "form",
      ...
      nextLabel: "opc.domain.wizard.next",
      showBack: false,
    });
  }
  ...
  steps.push({
    id: "confirm",
    title: "opc.domain.wizard.steps.confirm",
    description: "opc.domain.wizard.steps.confirmDesc",
    type: "confirm",
    canSkip: () => !hasInputs,
    nextLabel: "opc.domain.wizard.confirmExecute",
    prevLabel: "opc.domain.wizard.back",
    showBack: hasInputs,
  });

  steps.push({
    id: "execute",
    title: "opc.domain.wizard.steps.execute",
    description: "opc.domain.wizard.steps.executeDesc",
    type: "execute",
    showBack: false,
  });
  ...
  steps.push({
    id: "result",
    title: "opc.domain.wizard.steps.result",
    type: "result",
    showBack: false,
  });
    ...
    <div style={{ padding: "8px 0" }}>
      ...
      {workflow.version && (
        <Tag color="blue" style={{ marginBottom: 16 }}>
          {t("opc.domain.wizard.version", { version: workflow.version })}
        </Tag>
      )}
      {fields.length > 0 && (
        <div
        ...
        >
          <Text strong style={{ display: "block", marginBottom: 12 }}>
            {t("opc.domain.wizard.paramsTitle")}
          </Text>
          {fields.map((field) => {
            const rawValue = values[field.key];
            const displayValue = rawValue !== undefined && rawValue !== ""
              ? String(rawValue)
              : t("opc.domain.wizard.notFilled");
            return (
              <div
                key={field.key}
                style={{
...
function renderExecuteStep(t: (key: string) => string) {
  return (
    ...
    >
      ...
      <Text style={{ fontSize: 16 }}>{t("opc.domain.wizard.executing")}</Text>
    </div>
  );
}
        ...
        ? (
          <>
            ...
            <Title level={4} style={{ margin: 0, color: "#52c41a" }}>
              {t("opc.domain.wizard.resultSuccess")}
            </Title>
        ...
        ? (
          <>
            ...
            <Title level={4} style={{ margin: 0, color: "#ff4d4f" }}>
              {t("opc.domain.wizard.resultFailed")}
            </Title>
...
export function WorkflowWizard({ open, workflow, data, onClose }: WorkflowWizardProps) {
    ...
    execute: async () => {
      ...
      try {
        ...
        if (result.status === "completed" || result.status === "success") {
          ...
          setResultMessage(t("opc.domain.wizard.executeSuccess", { id: workflow.id }));
        } else {
          setResultStatus("failed");
          setResultMessage(t("opc.domain.wizard.executeFailed", { id: workflow.id }));
        }
      } catch {
        setExecuting(false);
        setResultStatus("failed");
  ...
  const nextLabel = currentStep?.nextLabel
    ...
    : isLastStep
    ? t("opc.domain.wizard.finish")
    : t("opc.domain.wizard.next");

  return (
    <Modal
      open={open}
      onCancel={onClose}
      title={t("opc.domain.wizard.title", {
