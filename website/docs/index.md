---
layout: home
title: AxAgent — Open-source AI Desktop Client & Gateway
titleTemplate: false

head:
  - - meta
    - name: description
      content: AxAgent is a free, open-source AI desktop client with built-in AI gateway. Connect OpenAI, Claude, Gemini, DeepSeek and more LLMs. MCP server support, knowledge base, privacy-first.
...
features:
  ...
  - icon: api
    ...
    details: Full Model Context Protocol support — stdio, SSE, StreamableHTTP. Connect external tools with one click. Built-in MCP tools like @axagent/fetch.
  - icon: cloud-server
    title: Built-in API Gateway
    details: Local OpenAI-compatible API server. Use AxAgent as the backend for Claude Code, Codex, Gemini CLI, and more. Key management, rate limiting, SSL/TLS.
  - icon: book
