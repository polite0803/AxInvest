...
//!
//! 替代 OpcDomainPackAdapter 中的静态配置数据（验证规则、KPI 定义、自动化规则、仪表盘等）。
//! 动态业务逻辑（validate、compute_kpis）保留在独立的 Service 中。

use serde::{Deserialize, Serialize};

use super::analytics::{KpiDefinition, KpiValue};
use super::automation::{AutomationAction, AutomationCondition, DomainPackAutomationRule};
use super::workflow::{
...
pub struct DomainPackConfig {
    ...
    // ── 自动化规则 ──
    pub automation_rules: Vec<DomainPackAutomationRule>,
...
pub fn accounting_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "accounting".to_string(),
        domain_pack_name: "会计与财务管理".to_string(),
        version: 1,
        validations: vec![
            ValidationDef {
...
pub fn finance_invest_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "finance_invest".to_string(),
        domain_pack_name: "金融投资".to_string(),
        version: 1,
        validations: vec![
            ValidationDef {
        ...
        requires_approval: false,
        workflow_steps: Vec::new(),
        automation_rules: vec![DomainPackAutomationRule::new(
            "finance_risk_alert",
            "风险预警",
...
pub fn software_dev_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "software_dev".to_string(),
        domain_pack_name: "软件开发".to_string(),
        version: 1,
        validations: vec![
            ValidationDef {
...
pub fn design_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "design".to_string(),
        domain_pack_name: "设计".to_string(),
        version: 1,
...
pub fn ai_research_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "ai_research".to_string(),
        domain_pack_name: "AI 研究与咨询".to_string(),
        version: 1,
        validations: vec![ValidationDef {
...
pub fn content_media_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "content_media".to_string(),
        domain_pack_name: "内容与媒体".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "title".to_string(),
            r#type: "required".to_string(),
        ...
        // （`domain_pack::runtime_schema`），此处不再保留第二份副本 ——
        // 两份词表历史上一度完全对不上（content_views/engagement_rate vs
        // publish_count/total_views），这正是本次改造要消灭的错位。
        // KPI **计算**在 Rust 侧按 key 注册（`domain_pack_kpi_service::KPI_SOURCE_REGISTRY`）。
        kpi_definitions: vec![],
        kpi_calculations: vec![],
        input_fields: vec![WorkflowInputField {
...
pub fn ecommerce_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "ecommerce".to_string(),
        domain_pack_name: "电子商务".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "sku".to_string(),
            r#type: "required".to_string(),
...
pub fn education_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "education".to_string(),
        domain_pack_name: "教育培训".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "title".to_string(),
            r#type: "required".to_string(),
...
pub fn geospatial_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "geospatial".to_string(),
        domain_pack_name: "地理信息".to_string(),
        version: 1,
        validations: vec![ValidationDef {
...
pub fn consulting_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "consulting".to_string(),
        domain_pack_name: "咨询".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "topic".to_string(),
            r#type: "required".to_string(),
...
pub fn project_management_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "project_management".to_string(),
        domain_pack_name: "项目管理".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "name".to_string(),
            r#type: "required".to_string(),
...
pub fn sales_growth_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "sales_growth".to_string(),
        domain_pack_name: "销售增长与营销".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "name".to_string(),
            r#type: "required".to_string(),
...
pub fn security_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "security".to_string(),
        domain_pack_name: "安全合规".to_string(),
        version: 1,
        validations: vec![ValidationDef {
...
pub fn game_dev_config() -> DomainPackConfig {
    DomainPackConfig {
        domain_pack_id: "game_dev".to_string(),
        domain_pack_name: "游戏开发".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "name".to_string(),
            r#type: "required".to_string(),
...
/// 获取所有域包配置
pub fn get_all_configs() -> Vec<DomainPackConfig> {
    vec![
        accounting_config(),
        ai_research_config(),
...
/// 按域包 ID 获取配置
pub fn get_config(domain_pack_id: &str) -> Option<DomainPackConfig> {
    let id = domain_pack_id.replace('-', "_");
    get_all_configs().into_iter().find(|c| c.domain_pack_id == id)
}

/// 列出所有域包 ID 和名称
pub fn list_industries() -> Vec<(String, String)> {
    get_all_configs().into_iter().map(|c| (c.domain_pack_id, c.domain_pack_name)).collect()
}

// ── 共享类型（迁移自 domain_pack/mod.rs） ──
...
pub struct DomainPackDashboard {
    pub domain_pack_id: String,
...
mod tests {
    ...
    /// **命名空间边界**：OPC 业务记录类型（`DomainPackConfig.entity_types`）与知识图谱实体类型
