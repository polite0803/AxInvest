...
fn load_runtime_kpi_meta(
    app_dir: Option<&Path>,
    domain_pack_id: &str,
) -> Option<domain_pack::runtime_schema::RuntimeKpiConfig> {
    let base = axagent_analysis_engine::opc::resolve_domain_packs_dir(app_dir);
    ...
    for dir_name in candidates {
        ...
        if let Some(bundle) = domain_pack::analysis_schema::load_domain_pack(&dir) {
...
pub(crate) fn declared_kpi_thresholds_from(
    meta: Option<&domain_pack::runtime_schema::RuntimeKpiConfig>,
    domain_pack_id: &str,
        ...
        match produced.remove(&def.key) {
            ...
            None => {
                let (availability, note) =
                    match domain_pack_kpi_service::kpi_source(domain_pack_id, &def.key) {
                        Some(domain_pack_kpi_service::KpiSource::NoDataSource(reason)) => {
...
pub async fn validate_entity(
    ...
    let errors = domain_pack_validator::validate_entity(domain_pack_id, entity_type, entity_data)
        .await
        .map_err(|e| {
...
pub async fn validate_batch(
    ...
    let results =
        domain_pack_validator::validate_batch(domain_pack_id, entities).await.map_err(|e| {
            String::from(crate::commands::error::ErrorResponse::from_error(
                e,
                crate::commands::error::ErrorCategory::Unrecoverable,
            ))
...
pub async fn compute_kpis(
    ...
    domain_pack_kpi_service::compute_kpis(domain_pack_id, &data_service, &time_range).await.map_err(
        |e| {
            String::from(crate::commands::error::ErrorResponse::from_error(
                e,
                crate::commands::error::ErrorCategory::Unrecoverable,
...
/// 回退 `domain_pack_config`（旧域包/未迁域包）。
pub fn get_kpi_definitions(
    app_dir: Option<&Path>,
    domain_pack_id: &str,
) -> Result<Vec<KpiDefinition>, String> {
    let config = domain_pack_config::get_config(domain_pack_id)
...
/// 获取域包启用的自动化规则
pub fn get_enabled_rules(domain_pack_id: &str) -> Result<Vec<DomainPackAutomationRule>, String> {
    let config = domain_pack_config::get_config(domain_pack_id)
        .ok_or_else(|| format!("域包配置不存在: {}", domain_pack_id))?;
...
pub async fn run_automation_rules(
    ...
    let ctx_map = opc_domain_pack_logic::context_to_hashmap(&context);
    let data_service: Arc<dyn OpcDataService> = Arc::new(DefaultDataService::new(db.clone()));
    let mut triggered = Vec::new();
    for rule in &rules {
        if opc_domain_pack_logic::evaluate_conditions(&rule.conditions, &ctx_map) {
            opc_domain_pack_logic::execute_rule_actions(Some(&data_service), rule, &context)
                .await
                .map_err(|e| {
                    String::from(crate::commands::error::ErrorResponse::from_error(
                        e,
...
pub async fn get_dashboard(
    ...
    domain_pack_id: &str,
    time_range: TimeRange,
) -> Result<DomainPackDashboard, String> {
    let config = domain_pack_config::get_config(domain_pack_id)
        .ok_or_else(|| format!("域包配置不存在: {}", domain_pack_id))?;
    ...
    Ok(DomainPackDashboard {
        domain_pack_id: domain_pack_id.to_string(),
        kpis,
        cards,
        summary: None,
        // 已判定（`Some`）；未判定语义见 `DomainPackDashboard::risk_level` 的文档。
...
pub fn list_industries() -> Vec<(String, String)> {
    domain_pack_config::list_industries()
}

/// 检查域包是否存在（从配置获取）
pub fn has_domain_pack(domain_pack_id: &str) -> bool {
    domain_pack_config::get_config(domain_pack_id).is_some()
}
...
pub async fn opc_validate_entity(
    ...
    let errors = validate_entity(db, &domain_pack_id, &entity_type, &entity_data).await?;
    Ok(serde_json::json!({
...
pub async fn opc_compute_kpis(
    ...
    let db = app_state.harness.db();
    let kpis = compute_kpis(db, &domain_pack_id, range).await?;
    Ok(serde_json::json!({
        "domainPackId": domain_pack_id,
        "kpis": kpis,
    }))
...
pub async fn opc_run_automation_rules(
    ...
    let db = app_state.harness.db();
    let triggered = run_automation_rules(db, &domain_pack_id, ctx).await?;
    Ok(serde_json::json!({
        "domainPackId": domain_pack_id,
...
#[tauri::command]
pub async fn opc_get_domain_pack_dashboard(
    app_state: State<'_, AppState>,
    domain_pack_id: String,
    days: Option<i64>,
) -> Result<serde_json::Value, String> {
    ...
    let db = app_state.harness.db();
    let dashboard =
        get_dashboard(Some(&app_state.app_data_dir), db, &domain_pack_id, range).await?;
    Ok(serde_json::json!({
        "domainPackId": domain_pack_id,
        "dashboard": dashboard,
    }))
...
#[tauri::command]
pub async fn opc_get_domain_pack_workflow_steps(
    _app_state: State<'_, AppState>,
    domain_pack_id: String,
) -> Result<serde_json::Value, String> {
    let steps = get_workflow_steps(&domain_pack_id)?;
...
#[tauri::command]
pub async fn opc_get_domain_pack_automation_rules(
    _app_state: State<'_, AppState>,
    domain_pack_id: String,
) -> Result<serde_json::Value, String> {
    let rules = get_enabled_rules(&domain_pack_id)?;
    Ok(serde_json::json!({
        "domainPackId": domain_pack_id,
        "rules": rules,
    }))
...
pub async fn opc_batch_validate_entities(
    ...
    let pairs: Vec<(String, serde_json::Value)> = entities
        ...
        .collect();
    let db = app_state.harness.db();
    let results = validate_batch(db, &domain_pack_id, &pairs).await?;
    Ok(serde_json::json!({
        "domainPackId": domain_pack_id,
...
pub async fn opc_get_kpi_definitions(
    ...
    let definitions = get_kpi_definitions(Some(&app_state.app_data_dir), &domain_pack_id)?;
    Ok(serde_json::json!({
        "domainPackId": domain_pack_id,
...
#[tauri::command]
pub async fn opc_has_domain_pack(
    _app_state: State<'_, AppState>,
    domain_pack_id: String,
) -> Result<serde_json::Value, String> {
    let exists = has_domain_pack(&domain_pack_id);
    Ok(serde_json::json!({
        "domainPackId": domain_pack_id,
        "exists": exists,
    }))
...
// ── Phase 1 数据接入命令（OpDomainPackClient 直读域包 analysis.yaml） ──

/// 构造域包数据客户端（db/cache/web/file 内建 vendor，无容器）
fn build_data_client(
    app_state: &AppState,
    domain_pack_id: &str,
) -> Result<OpDomainPackClient, String> {
    let dir = domain_pack_dir(Some(&app_state.app_data_dir), domain_pack_id)?;
    let config = crate::commands::opc_data::load_analysis_config(&dir)?;
    let db = app_state.harness.db();
    let mut vendors: std::collections::HashMap<String, std::sync::Arc<dyn OpDomainPackVendor>> =
    ...
    Ok(OpDomainPackClient::new(domain_pack_id.to_string(), sources, vendors))
}
...
#[tauri::command]
pub async fn opc_get_domain_pack_data(
    app_state: State<'_, AppState>,
    domain_pack_id: String,
    source_id: String,
    data_domain: String,
    ...
    let client = build_data_client(&app_state, &domain_pack_id)?;
    let data = client
...
#[tauri::command]
pub async fn opc_get_domain_pack_precheck(
    app_state: State<'_, AppState>,
    domain_pack_id: String,
) -> Result<serde_json::Value, String> {
    let client = build_data_client(&app_state, &domain_pack_id)?;
...
#[tauri::command]
pub async fn opc_get_domain_pack_health(
    app_state: State<'_, AppState>,
    domain_pack_id: String,
) -> Result<serde_json::Value, String> {
    let client = build_data_client(&app_state, &domain_pack_id)?;
...
pub async fn opc_execute_dynamic_workflow(
    ...
    crate::commands::opc_domain_pack_actions::opc_execute_workflow(
        app_state,
        domain_pack_id,
        workflow_id,
        days,
