...
//! v134: 为 `opc_demand_leads` 添加实现链路两列 —— 转化工作流 ID + 执行时间。
//!
//! ## Background
//!
...
pub async fn up(db: sea_orm::DatabaseConnection) -> Result<(), DbErr> {
    ...
    if is_pg {
            ...
            alter("ALTER TABLE opc_demand_leads ADD COLUMN linked_workflow_id TEXT").await
        {
            ...
            tracing::debug!("[v134] opc_demand_leads.linked_workflow_id 已存在，跳过");
        }
        if let Err(e) = alter("ALTER TABLE opc_demand_leads ADD COLUMN implemented_at BIGINT").await
        {
            ...
            tracing::debug!("[v134] opc_demand_leads.implemented_at 已存在，跳过");
        }
    }
    ...
    tracing::info!("[v134] Added linked_workflow_id + implemented_at to opc_demand_leads");
    Ok(())
}
...
mod tests {
    ...
    #[tokio::test]
    async fn v134_adds_columns_sqlite() {
