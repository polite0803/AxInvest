...
//! 基础域包适配器实现
//!
//! 提供通用的 CapabilityPackAdapter 实现，包含：
//! - 动态任务分解的基础逻辑
//! - 任务类型检测的关键词匹配
//! - 反思模板、进化约束、验收标准的配置注入
//!
...
use crate::capability_pack_adapters::{
    CapabilityPackAdapter,
    types::{
        AcceptanceCriterion, CapabilityPackContext, CapabilityPackLearningConfig,
        EvolutionConstraints, MissionType, PresetWorkflowStep, ReflectionTemplate,
    },
};
...
pub struct BaseCapabilityPackAdapter {
...
impl BaseCapabilityPackAdapter {
    ...
    pub fn with_learning_config(mut self, config: CapabilityPackLearningConfig) -> Self {
    ...
    pub fn from_config_json(
        ...
        let mut adapter = Self::new(domain_pack_id, capability_pack_name);
...
#[async_trait]
impl CapabilityPackAdapter for BaseCapabilityPackAdapter {
