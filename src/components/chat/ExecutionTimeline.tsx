...
export const ExecutionTimeline = React.memo(function ExecutionTimeline({ conversationId, maxItems = 50 }: ExecutionTimelineProps) {
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
            ...
            >
              ...
              <span style={{ marginLeft: 4 }}>
                {evt.type === "plan" ? t("chat.timeline.plan", "计划") : evt.type === "tool_call" ? t("chat.timeline.tool", "工具") : evt.type === "agent_pool" ? t("chat.timeline.agent", "智能体") : t("chat.timeline.status", "状态")}
              </span>
            </Tag>
            {/* Tool-specific icon */}
            ...
            <Tag
              color={evt.status === "completed" ? "green" : evt.status === "failed" ? "red" : evt.status === "running" ? "blue" : "default"}
              style={{ margin: 0, fontSize: 10, padding: "0 4px", lineHeight: "18px" }}
            >
              {evt.status === "completed" ? t("chat.timeline.completed", "完成") : evt.status === "failed" ? t("chat.timeline.failed", "失败") : evt.status === "running" ? t("chat.timeline.running", "运行中") : evt.status === "cancelled" ? t("chat.timeline.cancelled", "已取消") : t("chat.timeline.pending", "等待中")}
            </Tag>
          </div>
