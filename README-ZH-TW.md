...
<p align="center">
  <strong>跨平台 AI 桌面/行動用戶端 | 多智能體協作 | 本地優先</strong>
</p>
...
**AxAgent v2.0** 是一款功能全面的跨平台 AI 桌面/行動用戶端，整合了先進的 AI 智慧體能力和豐富的開發者工具。它支援多模型提供商、自主管道執行、視覺化工作流編排、本地知識管理、內建 API 閘道，覆蓋 **Windows / macOS / Linux / Android / iOS** 五大平台。
...
- **AI 影像生成** — 支援 DALL-E 3 和 Flux (Replicate)，多種尺寸預設（1:1/16:9/9:16/4:3），負面提示詞
- **模型智慧路由** — 按任務類型自動路由到不同模型（程式碼審查/摘要/翻譯），支援自訂路由規則
- **語音通話** — 基於 OpenAI Realtime API 的即時語音對話，支援連線/說話/聆聽狀態切換
...
- **智慧體池面板** — 視覺化子智慧體/Worker/工作流步驟的即時狀態，支援展開查看詳情
- **智慧體反思面板** — 任務執行後的品質評分、效率分析、錯誤模式和改進建議
- **專家選擇器** — 匯入/匯出/自訂專家角色，按類別篩選，支援內建預設和 Agency 專家
- **智慧體層級樹** — 視覺化展示智慧體之間的層級關係和協作拓撲
- **意圖分類器** — 自動識別使用者輸入的意圖類型，最佳化路由和回應策略
- **信念狀態管理** — 維護智慧體對當前上下文的理解狀態
- **目標評估器** — 評估任務目標的完成度和品質
- **上下文視窗管理** — 智慧管理對話上下文視窗，最佳化 token 使用
- **專案記憶** — 跨會話的專案級知識持久化與檢索
- **知識庫管理** — 知識庫的建立、更新、刪除和查詢
- **筆記系統** — 智慧體內的結構化筆記儲存與檢索
...
- **協作面板** — 即時協作會話管理，支援邀請碼分享、參與者角色（Owner/Editor/Viewer）、權限控制
- **會話分享** — 一鍵產生分享連結，支援終端/檔案/模型存取權限配置
...
- **Self-RAG** — 自檢索增強生成，智慧判斷是否需要檢索以及檢索結果的相關性
...
- **Git 提交面板** — 視覺化 Git diff 統計，AI 產生提交資訊，一鍵暫存和提交
- **工具推薦** — 基於上下文的智慧工具推薦引擎
- **工具編排** — 多工具協調執行和串流輸出
- **工具統計** — 工具使用頻率和效能統計
...
- **資訊圖渲染** — 支援資訊圖可視化展示
- **圖表解釋器** — AI 分析圖表資料並視覺化，支援長條圖/折線圖/圓餅圖/散點圖/面積圖，自動產生洞察和統計
- **差異檢視器** — 對話版本對比，支援逐檔案 Accept/Reject，自動語言偵測
- **上下文分類列** — 分段顯示上下文各分類（訊息/系統提示/知識/記憶/工具/技能）的 token 佔比
- **上下文圖譜** — ReactFlow 視覺化對話上下文關係圖（對話/模型/知識/記憶/MCP/搜尋/技能節點）
- **命令建議** — 輸入時自動推薦可用命令
- **引用管理器** — 追蹤和分類引用來源（Web/學術/Wikipedia/GitHub/文件/新聞/部落格/論壇），支援可信度評分
- **可信度徽章** — 五星評分視覺化來源可信度
...
- **雲端工作空間** — 支援 S3 和 WebDAV 雲端儲存同步，衝突偵測與解決，雙向同步
- **備份恢復** — 一鍵從歷史備份恢復
...
- **自適應佈局** — 桌面/平板/手機三檔自動適配（600px/900px 斷點），視窗縮放即時切換
...
- **夢境狀態指示器** — 即時顯示背景 Dream 鞏固執行狀態和結果（記憶條數/模式數）
...
- **自適應佈局** — 桌面/平板/手機三檔自動適配（600px/900px CSS 斷點，視窗縮放即時切換）
