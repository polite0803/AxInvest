...
impl PromptRegistry {
    ...
    pub fn format(key: &str, lang: PromptLang, args: &[&str]) -> String {
        ...
        for (i, a) in args.iter().enumerate() { r = r.replace(&format!("{{{}}}", i), a); }
        r
    }
...
// ═══ 中文 ═══
fn get_zh(key: &str) -> &'static str { match key {
"extraction.system_prompt" => r#"你是一个知识提取助手。你的任务是从对话转录中提取重要、可复用的知识。
...
"consolidation.system_prompt" => r#"你是一个知识整合助手。将多条相似的记忆合并为一条简洁的摘要。
...
"entity_extraction.system_prompt" => r#"你是一个实体和关系提取助手。从对话转录中提取关键实体及其关系。
...
"incremental_extract.system_prompt" => r#"你是一个知识提取助手。从最近的对话中提取新的知识。
规则：只提取之前不知道的新信息，重点关注用户的偏好、决策和项目背景。返回 JSON 数组，如果没有新知识返回 []。"#,

"incremental_extract.user_template" => "从以下最近的对话中提取新知识：\n\n{0}",

"conversation_summary.merge_template" => "你是一个对话摘要助手。请将以下新增对话内容合并到已有摘要中。\n\n已有摘要：\n{0}\n\n新增对话：\n{1}\n\n请输出合并后的摘要，保留所有重要信息，去除冗余。",

"conversation_summary.compress_template" => "你是一个对话摘要助手。请将以下对话历史压缩为简洁摘要。\n\n对话内容：\n{0}\n\n请保留关键决策、用户偏好、正在进行的工作和重要上下文。",

"title_generation.system_prompt" => "你是一个对话标题生成助手。根据对话内容生成一个简洁的标题（不超过 50 字符）。只返回标题文本。",
...
"compact.continuation_preamble" => "此会话从之前的对话续接而来。以下摘要覆盖了对话的早期部分。\n\n",
"compact.recent_messages_note" => "最近的消息被原样保留。",
"compact.resume_instruction" => "从上次中断的地方继续对话，不要再向用户提问。直接继续。",

"workflow_ai.generation_system" => "你是一个工作流设计助手。根据用户的描述，生成一个结构化的自动化工作流。输出 JSON 格式。",
...
// ═══ 英文 ═══
fn get_en(key: &str) -> &'static str { match key {
"extraction.system_prompt" => r#"You are a knowledge extraction assistant. Extract important, reusable knowledge from conversation transcripts.
...
"consolidation.system_prompt" => r#"You are a knowledge consolidation assistant. Merge multiple similar memories into one.
...
"entity_extraction.system_prompt" => r#"You are an entity extraction assistant. Extract entities and relationships.
...
"incremental_extract.system_prompt" => r#"You are a knowledge extraction assistant. Extract NEW knowledge only.
...
"conversation_summary.merge_template" => "Merge new conversation content into existing summary.\n\nExisting:\n{0}\n\nNew:\n{1}\n\nOutput merged summary, preserving important info.",

"conversation_summary.compress_template" => "Compress conversation history into concise summary.\n\nContent:\n{0}\n\nPreserve key decisions, preferences, ongoing work.",

"title_generation.system_prompt" => "Generate a concise title (max 50 chars) based on the conversation. Return only the title text.",
...
"compact.continuation_preamble" => "This session is being continued from a previous conversation that ran out of context. The summary below covers the earlier portion of the conversation.\n\n",
"compact.recent_messages_note" => "Recent messages are preserved verbatim.",
"compact.resume_instruction" => "Continue the conversation from where it left off without asking the user any further questions. Resume directly — do not acknowledge the summary, do not recap what was happening, and do not preface with continuation text.",

"workflow_ai.generation_system" => "You are a workflow design assistant. Generate a structured automation workflow based on the user's description. Output JSON format.",
...
mod tests {
    ...
    fn test_format_with_args() {
        let result = PromptRegistry::format("extraction.user_template", PromptLang::ZhCN, &["测试"]);
