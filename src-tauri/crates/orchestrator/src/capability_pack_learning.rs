...
use crate::capability_pack_adapters::types::{
...
请生成具体的改进措施列表。"#,
            capability_pack = request.domain_pack_id,
            target = request.target,
        );
    ...
    /// 从数据库或内存获取域包经验列表
    async fn get_capability_pack_experiences(&self, domain_pack_id: &str) -> Vec<RLExperience> {
        // 优先从数据库查询
        if let Some(ref store) = self.rl_store
    ...
    /// 获取域包经验数量
    async fn get_capability_pack_experience_count(&self, domain_pack_id: &str) -> u64 {
        if let Some(ref store) = self.rl_store
    ...
    pub async fn optimize_policy(
        ...
        let experiences = self.get_capability_pack_experiences(domain_pack_id).await;
        ...
        *counts.entry(domain_pack_id.to_string()).or_insert(0) += 1;
        ...
        // 如果有 rl_store，更新训练统计
        if let Some(ref store) = self.rl_store {
            let stats = axagent_harness::rl::RlCapabilityPackStats {
                domain_pack_id: domain_pack_id.to_string(),
    ...
    pub async fn run_reinforcement_learning(
        ...
        let pool_len = self.get_capability_pack_experience_count(domain_pack_id).await as usize;
    ...
    pub async fn get_capability_pack_experience_stats(
        &self,
        domain_pack_id: &str,
    ...
    pub async fn evaluate_gate(
        ...
        let capability_pack_gates = gates
            .get(domain_pack_id)
            .ok_or_else(|| format!("域包 {} 没有注册任何 Gate", domain_pack_id))?;
        let gate =
            capability_pack_gates.get(gate_id).ok_or_else(|| format!("Gate {} 不存在", gate_id))?;
    ...
    pub async fn register_hard_gate(
        ...
        let capability_pack_gates = gates.entry(domain_pack_id.to_string()).or_default();
        capability_pack_gates.insert(gate.id.clone(), gate);
        Ok(())
    }
    ...
    pub async fn get_hard_gates(&self, domain_pack_id: &str) -> Vec<HardGate> {
        ...
        gates
            .get(domain_pack_id)
            .map(|capability_pack_gates| capability_pack_gates.values().cloned().collect())
            .unwrap_or_default()
    }
...
impl Default for CapabilityPackLearningEngine {
    fn default() -> Self {
        Self::new()
    }
}
