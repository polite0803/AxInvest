...
//! OPC 域包专属操作后端命令
//!
//! 为 9 个垂直域包的操作入口提供后端配置和 prompt 生成。
...
use axagent_analysis_engine::opc::{CapabilityPackLearningManager, OpcCapabilityPackAnalysisRound};
...
/// 域包操作配置
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CapabilityPackActionConfig {
    /// 操作唯一标识
    pub key: String,
    /// 操作标签
    pub label: String,
...
/// 域包工作流配置
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CapabilityPackWorkflowConfig {
    /// 工作流唯一标识
    pub id: String,
    /// 工作流名称
    pub name: String,
...
/// 域包完整配置
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CapabilityPackConfig {
    /// 域包 ID
    pub id: String,
    /// 域包名称
    pub name: String,
    ...
    pub description: String,
    /// 域包专属操作列表
    pub actions: Vec<CapabilityPackActionConfig>,
    /// 域包专属工作流列表
    pub workflows: Vec<CapabilityPackWorkflowConfig>,
}

// ── 9 个域包配置定义 ──────────────────────────────────────────────

fn ai_research_config() -> CapabilityPackConfig {
...
fn software_dev_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn finance_invest_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn sales_growth_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn content_media_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn consulting_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        ...
        workflows: vec![
            CapabilityPackWorkflowConfig {
                ...
                version: "1.0".to_string(),
                template_id: "consulting_harness_workflow".to_string(),
            },
            CapabilityPackWorkflowConfig {
...
fn accounting_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn ecommerce_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn education_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn design_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        id: "design".to_string(),
        name: "设计".to_string(),
        icon: "🎨".to_string(),
...
fn project_management_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn security_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn geospatial_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
fn game_dev_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
/// 获取所有 14 个域包配置
pub fn get_all_capability_pack_configs() -> Vec<CapabilityPackConfig> {
    vec![
        ai_research_config(),
        software_dev_config(),
...
/// 根据域包 ID 获取配置
pub fn get_capability_pack_config(domain_pack_id: &str) -> Option<CapabilityPackConfig> {
    get_all_capability_pack_configs().into_iter().find(|c| c.id == domain_pack_id)
}

/// 根据域包 ID 和操作 key 获取操作配置
pub fn get_action_config(
    domain_pack_id: &str,
    action_key: &str,
) -> Option<CapabilityPackActionConfig> {
    get_capability_pack_config(domain_pack_id)
...
/// 接线：opc_capability_pack_bridge 的 opc_execute_capability_pack_action 工具 handler 调用。
pub fn execute_capability_pack_action(
    domain_pack_id: &str,
    action_key: &str,
    ...
    Ok(serde_json::json!({
        ...
        "executionId": format!("exec-{}-{}-{}", domain_pack_id, action_key, uuid::Uuid::new_v4().simple()),
    }))
}

/// 创建域包工作流实例（写操作逻辑封装）
///
/// 允许 Agent 根据域包模板创建一个新的工作流实例。
/// 在实际项目中，这将涉及数据库写入和工作流引擎初始化。
// 接线：opc_capability_pack_bridge 的 opc_create_capability_pack_workflow 工具 handler 调用
pub fn create_capability_pack_workflow(
    domain_pack_id: &str,
    workflow_id: &str,
...
/// 根据域包 ID 和工作流 ID 获取工作流配置
pub fn get_workflow_config(
    domain_pack_id: &str,
    workflow_id: &str,
) -> Option<CapabilityPackWorkflowConfig> {
    get_capability_pack_config(domain_pack_id)
...
/// 获取所有域包列表（简要信息）
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "获取所有域包列表")]
#[tauri::command]
pub async fn opc_list_industries() -> Result<serde_json::Value, String> {
    let configs = get_all_capability_pack_configs();
...
/// 获取域包完整配置（含操作和工作流）
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "获取域包完整配置")]
#[tauri::command]
pub async fn opc_get_capability_pack_config(
    capability_pack_id: String,
) -> Result<serde_json::Value, String> {
    let config = get_capability_pack_config(&capability_pack_id)
        .ok_or_else(|| format!("域包不存在: {capability_pack_id}"))?;
...
/// 获取域包（manifest 基本信息）
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "获取域包 manifest")]
#[tauri::command]
pub async fn opc_get_capability_pack(
    capability_pack_id: String,
) -> Result<serde_json::Value, String> {
    let config = get_capability_pack_config(&capability_pack_id)
        .ok_or_else(|| format!("域包不存在: {}", capability_pack_id))?;
...
/// 获取域包操作的执行配置（用于前端调用）
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "获取域包操作配置")]
#[tauri::command]
pub async fn opc_get_action_config(
    capability_pack_id: String,
    action_key: String,
) -> Result<serde_json::Value, String> {
    let action = get_action_config(&capability_pack_id, &action_key)
        .ok_or_else(|| format!("操作不存在: {capability_pack_id}/{action_key}"))?;
...
/// 获取域包工作流配置
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "获取域包工作流配置")]
#[tauri::command]
pub async fn opc_get_workflow_config(
    capability_pack_id: String,
    workflow_id: String,
) -> Result<serde_json::Value, String> {
    let workflow = get_workflow_config(&capability_pack_id, &workflow_id)
        .ok_or_else(|| format!("工作流不存在: {capability_pack_id}/{workflow_id}"))?;
...
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "构建域包上下文对话prompt")]
#[tauri::command]
pub async fn opc_build_capability_pack_prompt(
    capability_pack_id: String,
    action_key: String,
...
/// 获取域包所有操作列表
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "列出域包所有操作")]
#[tauri::command]
pub async fn opc_list_capability_pack_actions(
    capability_pack_id: String,
) -> Result<serde_json::Value, String> {
    let config = get_capability_pack_config(&capability_pack_id)
        .ok_or_else(|| format!("域包不存在: {capability_pack_id}"))?;
...
/// 获取域包所有工作流列表
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "列出域包所有工作流")]
#[tauri::command]
pub async fn opc_list_capability_pack_workflows(
    capability_pack_id: String,
) -> Result<serde_json::Value, String> {
    let config = get_capability_pack_config(&capability_pack_id)
        .ok_or_else(|| format!("域包不存在: {capability_pack_id}"))?;
...
/// 执行域包分析（使用新的分析引擎）
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "执行域包分析")]
#[tauri::command]
pub async fn opc_execute_analysis(
    ...
    // 域包 runtime.yaml 声明的风控阈值（**未声明 = 不启用**，不用默认值兜底）。
    // 阈值住在域包而不是 Rust：本项目的硬编码阈值是既有状态，不是该照抄的规范。
    let declared = crate::commands::opc_capability_pack_runtime::declared_kpi_thresholds(
        Some(&app_state.app_data_dir),
        &capability_pack_id,
    );
    let round = OpcCapabilityPackAnalysisRound::with_declared_thresholds(
        capability_pack_id.clone(),
        data_service,
        declared,
    );
...
/// 解析模板声明的 `hooks_config`（JSON 文本）为引擎类型。
///
/// **解析失败只 `warn` 后按「无钩子」处理，绝不上抛**：钩子是可选旁路
/// （`post_exec` 语义 = 结果已产生、不可回滚），一个 JSON 拼写错误不该让
/// 整条工作流执行失败。与 `src/commands/stock_workflow/core.rs:644-647`、`src/commands/stock_workflow/decision.rs:289-293`
/// 语义一致；与其唯一差异是**保留 warn 留痕**（那两处用 `.ok()` 静默吞错）。
///
/// 独立成函数的三个理由：
/// 1. **调用点短到不触及 rustfmt 链式断行**：原内联写法
///    `let hooks_config: Option<...> = template.hooks_config.as_ref().and_then(...)`
///    在 `src-tauri/rustfmt.toml`（`edition = "2024"`）下 `rustfmt --check` 报 diff。
///    合规形态见 `stock_pipeline/core.rs:290-291`（`=` 后断行、链保持一行、indent 8）；
/// 2. **纯函数可单测**（不需要 DB / 引擎），直接钉住「解析失败 ⇒ None，不 panic、不上抛」；
/// 3. `run_template_via_engine` 里「钩子来自模板声明」这一意图一行可见。
fn parse_hooks_config(
    template_id: &str,
    raw: Option<&str>,
) -> Option<axagent_harness::WorkflowHooksConfig> {
    // 模板未声明 / DB 列为 NULL ⇒ 无钩子（不是错误，不打日志）
    let raw_json = raw?;
    match serde_json::from_str(raw_json) {
        Ok(cfg) => Some(cfg),
        Err(e) => {
            tracing::warn!(
                "[opc-execute] 模板 {template_id} hooks_config 解析失败，按无钩子处理: {e}"
            );
            None
        },
    }
...
/// 这是域包工作流的唯一执行通道——所有 DAG 均来自 DB（种子化），不再运行时动态生成。
pub(crate) async fn run_template_via_engine(
    ...
    let domain_pack_id_normalized = domain_pack_id.replace('-', "_");
    ...
    // 创建工作流
    let wf_name = format!("opc-{domain_pack_id_normalized}-{template_id}");
    // 必须用 with_hooks 变体携带模板 hooks_config（形态同 src/commands/stock_workflow/core.rs:644-647）：
    // `create_workflow` 内部转调 `create_workflow_with_hooks(..., None)`，
    // 模板声明的 post_exec 会被静默丢弃 —— 引擎只在内存 Workflow 对象上读声明
    // （rt-workflow engine/mod.rs:942），为 None 时直接 return，连日志都不打。
    // 后果实证（2026-09-15）：`workflow-cm-literary-creation` 的
    // `post_exec: ["content-media-kpi-persist"]` 永不执行 ⇒ opc_kpi_records 永不落库
    // ⇒ 仪表盘创作类 KPI 恒为「暂无数据」。
    // 解析失败仅 warn 后按 None（见上方 parse_hooks_config）：钩子是可选旁路，
    // 一个 JSON 错误不该让整条工作流失败。
    let hooks_config = parse_hooks_config(template_id, template.hooks_config.as_deref());
    let workflow = engine
        .create_workflow_with_hooks(&wf_name, nodes.clone(), edges.clone(), hooks_config)
        .await
    ...
    let mut merged_vars = vec![
        Variable {
            name: "domain_pack_id".into(),
            var_type: "string".into(),
            value: serde_json::Value::String(domain_pack_id.to_string()),
            description: Some("域包 ID".into()),
            is_secret: false,
        },
        Variable {
    ...
    // P2：把域包归属注入执行上下文，供 post_exec 钩子
    // （`commands::opc_workflow_kpi_hook`，模板在 hooks_config.post_exec 里声明）
    // 写入 `opc_kpi_records.domain_pack_id`。
    //
    // 为什么这条通道可行（三处都已核实，无一处需要改 harness/引擎）：
    // - 引擎把 `options.input` 透传进 `HookExecContext.input`
    //   （post_exec 见 `rt-workflow engine/mod.rs:985`，pre_exec 见 `:2197-2202`）；
    // - 该模板没有 `input_schema`（RunOptions 用 `..Default::default()`，命令层也不传），
    //   故引擎 `:2014` 的 input_schema 校验分支不执行，多一个键不会被拒；
    // - 生产先例：股票管线用同一机制传业务键（`src/commands/stock_workflow/core.rs:799-814`
    //   的 `input: Some(json!({"stock_code": …}))`），实测已落进
    //   `workflow_executions.input_params`。
    //
    // 存**原始** domain_pack_id（连字符原样传递，如 `content-media`）：归一是消费端
    // （`canonical_domain_pack_id`）单点负责的事，这里再归一一次就等于两处各有一套规则
    // （正是本仓反复踩的「同一语义两处实现，迟早漂移」）。键名取
    // `crate::commands::constants::capability_pack::INPUT_KEY` 这一权威常量，不做字面量复写。
    let mut hook_input = serde_json::Map::new();
    hook_input.insert(
        crate::commands::constants::capability_pack::INPUT_KEY.to_string(),
        serde_json::Value::String(domain_pack_id.to_string()),
    );
    ...
    let opts = RunOptions {
        ...
        input: Some(serde_json::Value::Object(hook_input)),
        ..Default::default()
    };
...
/// 执行域包工作流
///
/// 三级查找：① 传入的 template_id（前端 wf-* / config 映射）→ ② 域包 harness 模板
/// （{domain_pack_id}_harness_workflow，seed 时写入 DB）→ ③ 从 adapter 一次性种子化到 DB
/// （用户之后可在工作流编辑器修改）再执行。全部走 rt-workflow 引擎，不再运行时动态生成 DAG。
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "执行域包工作流")]
#[tauri::command]
pub async fn opc_execute_workflow(
    ...
    use axagent_analysis_engine::opc::capability_pack_config as engine_config;

    let days = days.unwrap_or(30);

    // 归一化域包 ID（连字符转下划线）
    let domain_pack_id_normalized = capability_pack_id.replace('-', "_");

    // 1. 确定模板 ID：优先使用传入的 workflow_id，否则拼接默认模板 ID
    let template_id = if let Some(ref wf_id) = workflow_id {
        // 尝试从域包配置中找到对应的 template_id
        let capability_pack_config = get_capability_pack_config(&capability_pack_id);
        if let Some(config) = &capability_pack_config {
        ...
        format!("{domain_pack_id_normalized}_harness_workflow")
    };
    ...
    // 2b. 次选：域包 harness 模板（{domain_pack_id}_harness_workflow，seed 时写入 DB）
    let harness_template_id = format!("{domain_pack_id_normalized}_harness_workflow");
    ...
    // 3. 兜底：模板不存在时从域包配置一次性种子化到 DB（用户之后可在编辑器修改），再走 rt-workflow
    tracing::warn!(
        "[opc-execute] 模板 {} 与 {} 均不存在，从域包配置种子化后执行",
        template_id,
        harness_template_id
    );

    // 从 analysis-engine 的域包配置获取工作流模板数据
    let capability_pack_engine_config =
        engine_config::get_config(&domain_pack_id_normalized, Some(&app_state.app_data_dir))
            .ok_or_else(|| format!("域包配置不存在: {capability_pack_id}"))?;

    // 组合工具解析器
    let tool_resolver = |names: &[String]| -> Vec<axagent_harness::workflow_types::ToolDef> {
        use axagent_tools::tools::capability_pack_defs::{
            local_tool_defs, opc_tool_defs, stock_tool_defs,
        };
    ...
    let template_data =
        axagent_analysis_engine::opc::workflow::generate_capability_pack_template_data(
            &domain_pack_id_normalized,
            &capability_pack_engine_config,
            Some(&tool_resolver),
        );
...
/// 查询域包学习指标
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "查询域包学习指标")]
#[tauri::command]
pub async fn opc_get_learning_metrics(
    _app_state: State<'_, AppState>,
    capability_pack_id: String,
) -> Result<serde_json::Value, String> {
    let mut manager = CapabilityPackLearningManager::new();
    let engine = manager.get_or_create(&capability_pack_id).clone();
...
mod tests {
    ...
    // ── hooks_config 解析（L2 接线的关键前置）──────────────────────

    /// 模板声明了 `post_exec` ⇒ 必须原样解析出钩子名。
    ///
    /// 夹具用的正是 `workflow-cm-literary-creation` 的真实声明形态。
    /// 出处（2026-09-16 复核订正）：`src/commands/opc_workflows/seed_content_media.rs:201-203`
    /// ```
    /// 201: hooks_config: if template_id == "workflow-cm-literary-creation" {
    /// 202:     Some(literary_hooks_config())
    /// 203: } else {
    /// ```
    /// ⚠️ 行号会随该文件改动漂移；引用时请连**代码片段**一起给，别只给数字。
    #[test]
    fn parse_hooks_config_reads_post_exec() {
        let raw = r#"{"post_exec":["content-media-kpi-persist"]}"#;
        let cfg = parse_hooks_config("workflow-cm-literary-creation", Some(raw))
            .expect("合法 JSON 必须解析成功");
        assert_eq!(cfg.post_exec, vec!["content-media-kpi-persist".to_string()]);
        assert!(!cfg.is_empty(), "有 post_exec 声明时不应被视为空钩子");
    }

    /// 模板未声明 / DB 列为 NULL ⇒ `None`（**不是**空配置对象）。
    #[test]
    fn parse_hooks_config_none_when_absent() {
        assert!(parse_hooks_config("t", None).is_none());
    }

    /// **坏 JSON 必须降级成 `None`，不得上抛、不得 panic** ——
    /// 钩子是可选旁路，一个 JSON 拼写错误不该让整条工作流执行失败
    /// （team-lead 明确约束：解析失败 warn 后按 None，不用 `?` 上抛）。
    #[test]
    fn parse_hooks_config_degrades_on_bad_json() {
        assert!(parse_hooks_config("t", Some("not json at all")).is_none());
        assert!(parse_hooks_config("t", Some("{")).is_none());
        // 合法 JSON 但字段类型不符（post_exec 不是数组）同样降级
        assert!(parse_hooks_config("t", Some(r#"{"post_exec":"x"}"#)).is_none());
    }

    #[test]
    fn test_get_all_capability_pack_configs() {
        let configs = get_all_capability_pack_configs();
        assert_eq!(configs.len(), 14, "应有 14 个域包配置");
    }

    #[test]
    fn test_get_capability_pack_config() {
        let config = get_capability_pack_config("ai-research").expect("AI 研究域包应存在");
    ...
    #[test]
    fn test_build_capability_pack_prompt_with_input() {
    ...
    #[test]
    fn test_unknown_capability_pack_returns_none() {
        assert!(get_capability_pack_config("nonexistent").is_none());
    }
...
/// 域包内学习配置文件约定名
pub const DOMAIN_PACK_LEARNING_FILE: &str = "learning.yaml";

/// 域包内学习配置文件名：manifest 声明的 `learning` 字段（缺省 learning.yaml）。
///
/// manifest 缺失/解析失败时回退 [`DOMAIN_PACK_LEARNING_FILE`]，保持旧域包行为。
fn learning_file_for_pack(pack_dir: &std::path::Path) -> String {
    use axagent_analysis_engine::opc::capability_pack::read_manifest;
    read_manifest(pack_dir)
        .map(|m| m.learning)
        .unwrap_or_else(|| DOMAIN_PACK_LEARNING_FILE.to_string())
}

/// 学习配置文件路径解析：
///
/// 只读域包内 `config/opc/domain_packs/{dir_id}/{manifest.learning}`（缺省 learning.yaml）。
/// `dir_id` 由 `domain_pack_id` 连字符转下划线（`finance-invest` → `finance_invest`）。
///
/// 查找顺序：
/// 1. `app_dir` 下的同步副本（生产/服务模式）
/// 2. 仓库根相对路径（开发模式 CWD = 仓库根）
fn capability_pack_learning_config_path(
    domain_pack_id: &str,
    app_dir: Option<&std::path::Path>,
) -> Option<std::path::PathBuf> {
    let dir_id = domain_pack_id.replace('-', "_");

    // 1) 尝试 app_dir 路径
    if let Some(path) = app_dir {
        let pack_dir = path
            .join(axagent_analysis_engine::opc::capability_pack::CAPABILITY_PACKS_DIR)
            .join(&dir_id);
        let candidate = pack_dir.join(learning_file_for_pack(&pack_dir));
        if candidate.is_file() {
            tracing::debug!(
                "[capability_pack-learning] found via app_dir: {}",
                candidate.display()
            );
    ...
    if let Some(repo_root) = crate::commands::opc_workflows::find_repo_config_dir(
        axagent_analysis_engine::opc::capability_pack::CAPABILITY_PACKS_DIR,
    ) {
        let pack_dir = repo_root.join(&dir_id);
        let fallback = pack_dir.join(learning_file_for_pack(&pack_dir));
        if fallback.is_file() {
            tracing::debug!(
                "[capability_pack-learning] found via repo_root: {}",
                fallback.display()
            );
    ...
    tracing::warn!(
        "[capability_pack-learning] learning.yaml not found for {domain_pack_id} (app_dir={:?})",
...
pub struct CapabilityPackLearningConfigView {
    pub version: u32,
...
impl CapabilityPackLearningConfigView {
    /// 创建默认的学习配置（当 learning.yaml 不存在时使用）
    fn default_for(domain_pack_id: &str) -> Self {
        let capability_pack_names: std::collections::HashMap<&str, &str> = [
            ("ai-research", "人工智能研究"),
            ("software-dev", "软件开发"),
            ("finance-invest", "金融投资"),
            ("sales-growth", "销售增长"),
        ...
        let capability_pack_name =
            capability_pack_names.get(domain_pack_id).unwrap_or(&domain_pack_id).to_string();
...
/// [`capability_pack_learning_config_path`]（域包内 learning.yaml 优先）。
pub fn get_capability_pack_learning_config(
    domain_pack_id: &str,
    app_dir: Option<&std::path::Path>,
) -> Option<CapabilityPackLearningConfigView> {
    let capability_pack_names: std::collections::HashMap<&str, &str> = [
        ("ai-research", "人工智能研究"),
        ("software-dev", "软件开发"),
        ("finance-invest", "金融投资"),
    ...
    let config_path = capability_pack_learning_config_path(domain_pack_id, app_dir)?;
    ...
    Some(CapabilityPackLearningConfigView {
        version,
        domain_pack_id: domain_pack_id.to_string(),
        capability_pack_name,
        reflection_enabled,
...
/// 获取所有域包学习配置
///
/// v1.1 域包独立版：动态扫描域包目录（`config/opc/domain_packs/*/learning.yaml`），
/// 新增域包无需改代码即可自动出现（消灭硬编码 9 域包列表）。
pub fn get_all_capability_pack_learning_configs(
    app_dir: Option<&std::path::Path>,
) -> Vec<CapabilityPackLearningConfigView> {
    let base = axagent_analysis_engine::opc::capability_pack::resolve_capability_packs_dir(app_dir);
    let mut out = Vec::new();
    if let Ok(rd) = std::fs::read_dir(&base) {
        for entry in rd.filter_map(Result::ok) {
            ...
            // 域包目录名下划线 → domain_pack_id 连字符（ai_research → ai-research）
            let domain_pack_id = dir_name.replace('_', "-");
            if let Some(cfg) = get_capability_pack_learning_config(&domain_pack_id, app_dir) {
                out.push(cfg);
            }
        }
    }
    out.sort_by(|a, b| a.domain_pack_id.cmp(&b.domain_pack_id));
    out
}
...
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "获取域包学习配置")]
#[tauri::command]
pub async fn opc_get_learning_config(
    state: State<'_, AppState>,
    capability_pack_id: String,
) -> Result<serde_json::Value, String> {
    let config = get_capability_pack_learning_config(
        &capability_pack_id,
        Some(&state.app_data_dir),
    )
    .unwrap_or_else(|| {
        tracing::warn!(
            "[opc_get_learning_config] 域包学习配置不存在，返回默认配置: {capability_pack_id}"
        );
        CapabilityPackLearningConfigView::default_for(&capability_pack_id)
    });
    serde_json::to_value(config).map_err(|e| {
        String::from(crate::commands::error::ErrorResponse::from_error(
            e,
...
/// 获取所有域包学习配置
#[agent_command(domain = "automation", safety = Safe, call_mode = StateInput, description = "获取所有域包学习配置")]
#[tauri::command]
pub async fn opc_list_learning_configs(
    state: State<'_, AppState>,
) -> Result<serde_json::Value, String> {
    let configs = get_all_capability_pack_learning_configs(Some(&state.app_data_dir));
...
pub async fn opc_reflect_on_workflow(
    ...
    let config =
        get_capability_pack_learning_config(&capability_pack_id, Some(&state.app_data_dir))
            .ok_or_else(|| format!("域包学习配置不存在: {capability_pack_id}"))?;

    if !config.reflection_enabled {
        return Err(format!("域包 {} 的反思功能未启用", capability_pack_id));
    }

    // 获取域包适配器
    let registry = state.learning.capability_pack_adapter_registry.lock().await;
    let adapter = registry
        .get(&capability_pack_id)
        .ok_or_else(|| format!("域包适配器不存在: {capability_pack_id}"))?;
    ...
    let request = axagent_orchestrator::ReflectionRequest {
        domain_pack_id: capability_pack_id.clone(),
    ...
    // 调用学习引擎
    let engine = &state.learning.capability_pack_learning_engine;
...
pub async fn opc_evolve_workflow(
    ...
    if !config.evolution_enabled {
        return Err(format!("域包 {} 的进化功能未启用", capability_pack_id));
    }
...
pub async fn opc_run_self_improvement(
    ...
    if !config.self_improvement_enabled {
        return Err(format!("域包 {} 的自我改进功能未启用", capability_pack_id));
    }
...
/// 触发域包学习闭环（自动模式）
///
/// 根据域包配置自动触发反思、进化和自我改进。
/// 通常在工作流执行完成后调用，实现 "执行 → 反思 → 进化 → 改进" 的闭环。
#[agent_command(domain = "automation", safety = Caution, call_mode = StateInput, description = "触发域包学习闭环")]
#[tauri::command]
pub async fn opc_trigger_capability_pack_learning(
    state: State<'_, AppState>,
    capability_pack_id: String,
    workflow_id: String,
    workflow_result: serde_json::Value,
    ...
    if rl_config.enabled {
        ...
        let full_rl_config =
            load_rl_config(&capability_pack_id, Some(&state.app_data_dir)).unwrap_or(rl_config);
...
/// [`get_capability_pack_learning_config`] 一致（域包内 learning.yaml 优先）。
...
/// 获取经验池统计信息
/// P2-7：支持按域包过滤（domain_pack_id 可选；此前参数被静默忽略）
...
pub async fn opc_get_rl_stats(
    ...
    let engine = &state.learning.capability_pack_learning_engine;
    let stats = match capability_pack_id.filter(|s| !s.trim().is_empty()) {
        Some(id) => engine.get_capability_pack_experience_stats(&id).await,
...
pub async fn opc_trigger_rl_optimization(
    ...
    let rl_config = load_rl_config(&capability_pack_id, Some(&state.app_data_dir))
        .ok_or_else(|| format!("域包 {} 的 RL 配置不存在", capability_pack_id))?;

    if !rl_config.enabled {
        return Err(format!("域包 {} 的强化学习未启用", capability_pack_id));
    }

    let engine = &state.learning.capability_pack_learning_engine;
    let result = engine.optimize_policy(&capability_pack_id, &rl_config).await.map_err(|e| {
        String::from(crate::commands::error::ErrorResponse::from_error(
            e,
            crate::commands::error::ErrorCategory::Unrecoverable,
        ))
