...
// ⚠️ 本文件当前未参与编译（agent/lib.rs 未声明，勿删）。
// 被取代：现行实现是 shared_blackboard.rs（406 行，DTO/trait 已收口到 axagent-harness::multi_agent）。
// 本文件是 581 行旧版独立实现（自带 broadcast/EntryPriority/TTL），功能与收口版重叠。
// 注意：analysis-engine 另有 blackboard.rs（被主 crate stock_workflow 消费），三者需裁决归属。
