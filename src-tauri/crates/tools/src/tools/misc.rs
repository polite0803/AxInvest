...
impl Tool for ExportWordTool {
    ...
    fn description(&self) -> &str {
        "将 Markdown 导出为 Word (.docx) 文件，支持标题、列表、引用等格式。"
    }
    ...
    async fn call(&self, input: Value, _ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        for line in markdown.lines() {
            ...
            if trimmed.is_empty() {
                ...
                doc = doc.add_paragraph(
                    Paragraph::new().add_run(Run::new().add_text(s).size(36).bold()),
                );
            } else if let Some(s) = trimmed.strip_prefix("## ") {
                doc = doc.add_paragraph(
                    Paragraph::new().add_run(Run::new().add_text(s).size(28).bold()),
                );
            } else if let Some(s) = trimmed.strip_prefix("### ") {
                doc = doc.add_paragraph(
                    Paragraph::new().add_run(Run::new().add_text(s).size(24).bold()),
                );
            ...
            {
                doc = doc.add_paragraph(
                    Paragraph::new().add_run(Run::new().add_text(format!("• {}", s))),
                );
            } else if let Some(s) = trimmed.strip_prefix("> ") {
                doc = doc.add_paragraph(
                    Paragraph::new().add_run(Run::new().add_text(s).italic().color("666666")),
                );
            } else {
                doc = doc
    ...
    async fn call(&self, input: Value, _ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        match std::fs::read_to_string(&file_path) {
            ...
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => Ok(ToolResult::success(format!(
                "记忆文件 '{}' 在 {} 中尚不存在。使用 WorkspaceWrite 创建。",
...
impl Tool for WorkspaceWriteTool {
    ...
    async fn call(&self, input: Value, _ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        Ok(ToolResult::success(format!(
            "记忆文件 '{}' 已{}到 {}",
            safe_name, action, workspace_path
        )))
    }
}
...
impl Tool for DetectEncodingTool {
    ...
    fn description(&self) -> &str {
        "检测文件的字符编码（UTF-8/16 BOM、ASCII 比例分析）。"
    }
    fn input_schema(&self) -> Value {
        serde_json::json!({"type":"object","properties":{"path":{"type":"string"}},"required":["path"]})
    }
...
impl Tool for Base64ImageTool {
    ...
    async fn call(&self, input: Value, _ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        let bytes = std::fs::read(path).map_err(|e| ToolError::execution_failed(e.to_string()))?;
        use base64::Engine;
        ...
        match action {
            ...
            "list" => {
                ...
                if checkpoints.is_empty() {
                    return Ok(ToolResult::success("暂无检查点。使用 action='save' 创建。"));
                }
                let lines: Vec<String> =
                    std::iter::once(format!("检查点 ({}):", checkpoints.len()))
                        .chain(
                            checkpoints
                                .iter()
                                .map(|(id, lbl, ts)| format!("  {} -- {} ({})", id, lbl, ts)),
                        )
                        .collect();
            ...
            "restore" => {
                if ckpt_id.is_empty() {
                    return Ok(ToolResult::error("Error: restore 操作需要 checkpoint_id"));
                }
                let checkpoints = CHECKPOINTS
                    .lock()
                    .map_err(|e| ToolError::execution_failed(e.to_string()))?;
            ...
            _ => {
                Ok(ToolResult::error(format!("未知操作: {}。使用 save、list 或 restore。", action)))
            },
        }
    }
}
...
impl Tool for AgentRememberTool {
    ...
    async fn call(&self, input: Value, _ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        if value.is_empty() {
            return Ok(ToolResult::error("Error: value 是必需的"));
        }
...
impl Tool for DifySearchTool {
    ...
    async fn call(&self, input: Value, _ctx: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        if base_id.is_empty() {
            return Ok(ToolResult::error("Error: base_id 是必需的"));
        }
        if query.is_empty() {
            return Ok(ToolResult::error("Error: query 是必需的"));
        }
