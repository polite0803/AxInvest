...
async fn seed_profiles(
    ...
    for (expert_key, name, _) in experts {
        ...
        let am = agent_profiles::ActiveModel {
            ...
            source: Set("opc-domain_pack".into()),
            tags: Set(None),
...
pub async fn seed_ai_research_agents(db: &DatabaseConnection) -> Result<(), String> {
    seed_experts(db, AI_RESEARCH_EXPERTS, "opc-domain_pack", "#722ed1").await?;
    seed_profiles(db, AI_RESEARCH_EXPERTS, AI_RESEARCH_PROFILE_TOOLS, "opc-domain_pack", "🤖")
        .await?;
...
pub async fn seed_all_domain_pack_agents(db: &DatabaseConnection) -> Result<(), String> {
    // 1. 会计与财务管理
    seed_domain_pack_experts_inner(
        db,
        "accounting",
        domain_pack_experts::ACCOUNTING_EXPERTS,
        domain_pack_experts::ACCOUNTING_PROFILE_TOOLS,
        "opc-domain_pack",
        "#1890ff",
        "📊",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "finance-invest",
        domain_pack_experts::FINANCE_INVEST_EXPERTS,
        domain_pack_experts::FINANCE_INVEST_PROFILE_TOOLS,
        "opc-domain_pack",
        "#faad14",
        "💰",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "game-dev",
        domain_pack_experts::GAME_DEV_EXPERTS,
        domain_pack_experts::GAME_DEV_PROFILE_TOOLS,
        "opc-domain_pack",
        "#52c41a",
        "🎮",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "design",
        domain_pack_experts::DESIGN_EXPERTS,
        domain_pack_experts::DESIGN_PROFILE_TOOLS,
        "opc-domain_pack",
        "#eb2f96",
        "🎨",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "ecommerce",
        domain_pack_experts::ECOMMERCE_EXPERTS,
        domain_pack_experts::ECOMMERCE_PROFILE_TOOLS,
        "opc-domain_pack",
        "#13c2c2",
        "🛒",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "education",
        domain_pack_experts::EDUCATION_EXPERTS,
        domain_pack_experts::EDUCATION_PROFILE_TOOLS,
        "opc-domain_pack",
        "#2f54eb",
        "📚",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "geospatial",
        domain_pack_experts::GEOSPATIAL_EXPERTS,
        domain_pack_experts::GEOSPATIAL_PROFILE_TOOLS,
        "opc-domain_pack",
        "#389e0d",
        "🗺️",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "consulting",
        domain_pack_experts::CONSULTING_EXPERTS,
        domain_pack_experts::CONSULTING_PROFILE_TOOLS,
        "opc-domain_pack",
        "#722ed1",
        "💼",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "project-management",
        domain_pack_experts::PROJECT_MANAGEMENT_EXPERTS,
        domain_pack_experts::PROJECT_MANAGEMENT_PROFILE_TOOLS,
        "opc-domain_pack",
        "#fa8c16",
        "📋",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "sales-growth",
        domain_pack_experts::SALES_GROWTH_EXPERTS,
        domain_pack_experts::SALES_GROWTH_PROFILE_TOOLS,
        "opc-domain_pack",
        "#f5222d",
        "📈",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "security",
        domain_pack_experts::SECURITY_EXPERTS,
        domain_pack_experts::SECURITY_PROFILE_TOOLS,
        "opc-domain_pack",
        "#000000",
        "🔒",
    )
    ...
    seed_domain_pack_experts_inner(
        db,
        "software-dev",
        domain_pack_experts::SOFTWARE_DEV_EXPERTS,
        domain_pack_experts::SOFTWARE_DEV_PROFILE_TOOLS,
        "opc-domain_pack",
        "#2f54eb",
        "💻",
    )
...
async fn seed_domain_pack_experts_inner(
    db: &DatabaseConnection,
    ...
    tracing::info!("[opc-domain-pack] {domain_pack_name} 域包专家种子化完成");
    Ok(())
}
