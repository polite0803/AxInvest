...
export function TopicGroupDivider({
  conversationId,
  group,
}: TopicGroupDividerProps) {
  ...
  const handleRename = () => {
    if (label.trim()) {
      renameGroup(conversationId, group.id, label.trim());
    }
    setEditing(false);
  };
  ...
  const menuItems = {
    items: [
      {
        key: "rename",
        label: t("topicGroup.rename"),
        icon: <Edit3 size={12} />,
      },
      {
        key: "merge",
        label: t("topicGroup.mergeUp"),
        icon: <GitBranch size={12} />,
      },
      {
        key: "delete",
        label: t("topicGroup.delete"),
        icon: <Trash2 size={12} />,
        danger: true,
      },
    ],
