...
export function useChatViewMessages({
    ...
    for (const msg of deferredActiveMessages) {
      ...
      if (msg.role === "system" && msg.content === "<!-- context-clear -->") {
        ...
        const item = cached?.signature === signature
          ? cached.item
          : { key: msg.id, role: "context-clear", content: msg.id, variant: "borderless" as const };
      ...
      if (msg.role === "system" && msg.content === "<!-- context-compressed -->") {
        ...
        const item = cached?.signature === signature
          ? cached.item
          : { key: msg.id, role: "context-compressed", content: msg.id, variant: "borderless" as const };
        nextCache.set(msg.id, { signature, item });
        nextItems.push(item);
        continue;
      }
      ...
      if (nextCache.has(stableKey)) { continue; }
  ...
  const allBubbleItems = useMemo(() => {
    ...
    if (compressing) {
      items = [
        ...items,
        { key: "__compressing__", role: "context-compressing", content: "", variant: "borderless" as const },
      ];
    }
    return items;
  ...
  const aiContentNodesById = useMemo(() => {
    ...
    for (const item of bubbleItems) {
      if (item.role !== "ai" || typeof item.content !== "string") { continue; }
      ...
      if (shouldRenderFromContent) { continue; }
    ...
    for (const messageId of Array.from(cache.keys())) {
      if (!next.has(messageId)) { cache.delete(messageId); }
    }
    return next;
