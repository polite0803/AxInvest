...
function actionLabel(action: string): string {
  switch (action) {
    case "SELL_NOW": return "立即卖出";
    case "SELL_AT_LIMIT": return "挂限价卖出";
    case "SET_STOP_LOSS": return "设置止损";
    case "HOLD": return "继续持有";
    case "CONSIDER_ADD": return "考虑加仓";
...
function actionColor(action: string): string {
  switch (action) {
    case "SELL_NOW": return "var(--sa-green)";
    case "SELL_AT_LIMIT": return "#fa8c16";
    case "SET_STOP_LOSS": return "#faad14";
    case "HOLD": return "var(--sa-blue)";
    case "CONSIDER_ADD": return "var(--sa-red)";
  ...
  return (
    ...
    >
      {summary.recommendations.slice(0, 10).map((rec) => (
        <RecommendationCard key={rec.stockCode} rec={rec} />
      ))}
    </Card>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-3 mb-1" style={{ color: "var(--muted)" }}>
        ...
        <span>现价 <b>{rec.currentPrice.toFixed(2)}</b></span>
      ...
      <div className="flex gap-1 flex-wrap">
        ...
        {rec.signals.length > 4 && (
          <span style={{ color: "var(--muted)", fontSize: 9 }}>+{rec.signals.length - 4}</span>
        )}
      </div>
    </div>
  );
