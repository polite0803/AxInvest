//! 域包配置服务 — 从代码硬编码迁移到 Rust 常量配置
//!
//! 替代 OpcCapabilityPackAdapter 中的静态配置数据（验证规则、KPI 定义、自动化规则、仪表盘等）。
...
use super::automation::{AutomationAction, AutomationCondition, CapabilityPackAutomationRule};
use super::capability_pack::runtime_schema;
use super::workflow::{
    DashboardCardDef, KpiCalculationDef, ValidationDef, WorkflowInputField, WorkflowStepDef,
};

/// 域包配置 — 静态数据（可序列化、可存储到配置表）
...
impl CapabilityPackConfig {
    pub fn new(domain_pack_id: impl Into<String>, capability_pack_name: impl Into<String>) -> Self {
        Self {
            domain_pack_id: domain_pack_id.into(),
            capability_pack_name: capability_pack_name.into(),
            version: 1,
            validations: Vec::new(),
...
pub fn accounting_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "accounting".to_string(),
        capability_pack_name: "会计与财务管理".to_string(),
        version: 1,
        validations: vec![
            ValidationDef {
        ...
        automation_rules: vec![
            CapabilityPackAutomationRule::new(
...
pub fn finance_invest_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        ...
        requires_approval: false,
        workflow_steps: Vec::new(),
        automation_rules: vec![CapabilityPackAutomationRule::new(
            "finance_risk_alert",
            "风险预警",
...
pub fn software_dev_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
...
pub fn design_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "design".to_string(),
        capability_pack_name: "设计".to_string(),
        version: 1,
...
pub fn ai_research_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "ai_research".to_string(),
        capability_pack_name: "AI 研究与咨询".to_string(),
        version: 1,
        validations: vec![ValidationDef {
...
pub fn content_media_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "content_media".to_string(),
        capability_pack_name: "内容与媒体".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "title".to_string(),
            r#type: "required".to_string(),
        ...
        // KPI **元数据**（key/name/unit/metric_type）与展示卡片以域包
        // `config/opc/domain_packs/content_media/runtime.yaml` 为唯一权威
        // （`capability_pack::runtime_schema`），此处不再保留第二份副本 ——
        // 两份词表历史上一度完全对不上（content_views/engagement_rate vs
        // publish_count/total_views），这正是本次改造要消灭的错位。
        // KPI **计算**在 Rust 侧按 key 注册（`capability_pack_kpi_service::KPI_SOURCE_REGISTRY`）。
        kpi_definitions: vec![],
        kpi_calculations: vec![],
        input_fields: vec![WorkflowInputField {
...
pub fn ecommerce_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "ecommerce".to_string(),
        capability_pack_name: "电子商务".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "sku".to_string(),
            r#type: "required".to_string(),
...
pub fn education_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "education".to_string(),
        capability_pack_name: "教育培训".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "title".to_string(),
            r#type: "required".to_string(),
...
pub fn geospatial_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "geospatial".to_string(),
        capability_pack_name: "地理信息".to_string(),
        version: 1,
        validations: vec![ValidationDef {
...
pub fn consulting_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "consulting".to_string(),
        capability_pack_name: "咨询".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "topic".to_string(),
            r#type: "required".to_string(),
...
pub fn project_management_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "project_management".to_string(),
        capability_pack_name: "项目管理".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "name".to_string(),
            r#type: "required".to_string(),
...
pub fn sales_growth_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "sales_growth".to_string(),
        capability_pack_name: "销售增长与营销".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "name".to_string(),
            r#type: "required".to_string(),
...
pub fn security_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "security".to_string(),
        capability_pack_name: "安全合规".to_string(),
        version: 1,
        validations: vec![ValidationDef {
...
pub fn game_dev_config() -> CapabilityPackConfig {
    CapabilityPackConfig {
        domain_pack_id: "game_dev".to_string(),
        capability_pack_name: "游戏开发".to_string(),
        version: 1,
        validations: vec![ValidationDef {
            field: "name".to_string(),
            r#type: "required".to_string(),
...
/// 获取所有域包配置
pub fn get_all_configs() -> Vec<CapabilityPackConfig> {
    vec![
        accounting_config(),
        ai_research_config(),
...
///
/// 组装策略（阶段 A2 数据化）：
/// - 基线：现有 14 个内置函数（`get_all_configs`）提供**全部字段**的既有值；
/// - 覆盖：域包 `runtime.yaml` 已落盘且与 `CapabilityPackConfig` **同源**的段
///   （`automation_rules` / `entity_types`）非空则覆盖 —— 满足「接入即消费」红线
///   （`get_enabled_rules` / `run_automation_rules` 都经此处读到 YAML 规则）。
///
/// ⚠ `validations` / `workflow_steps` 段与 `CapabilityPackConfig` **形状不同源**（分别是
/// `RuntimeValidation` / `RuntimeWorkflowStep`，贴近 validator / 含 id），不在本处覆盖，
/// 由 A3（validator 接入）与阶段 B（模板化）各自消费，避免误合并。
pub fn get_config(domain_pack_id: &str, app_dir: Option<&Path>) -> Option<CapabilityPackConfig> {
    let id = domain_pack_id.replace('-', "_");
    let mut config = get_all_configs().into_iter().find(|c| c.domain_pack_id == id)?;

    // 读取域包 runtime.yaml（失败静默，走既有逻辑）
    let base = super::capability_pack::resolve_capability_packs_dir(app_dir);
    let dir = base.join(&id);
    if dir.is_dir() {
        if let Some(rt) = runtime_cfg_for(&dir) {
            if !rt.automation_rules.is_empty() {
                config.automation_rules = rt.automation_rules;
            }
            if !rt.entity_types.is_empty() {
                config.entity_types = rt.entity_types;
            }
        }
    }
    Some(config)
...
/// 读取单个域包目录的 `runtime.yaml`（阶段 A2 组装的数据源）。
/// 文件缺失 / 解析失败返回 `None`（调用方回退既有逻辑）。
fn runtime_cfg_for(dir: &Path) -> Option<runtime_schema::RuntimeKpiConfig> {
    let raw = std::fs::read_to_string(dir.join("runtime.yaml")).ok()?;
    match serde_yaml::from_str(&raw) {
        Ok(cfg) => Some(cfg),
        Err(e) => {
            tracing::warn!("[domain-pack-config] {} runtime 解析失败: {e}", dir.display());
            None
        },
    }
}
...
pub fn list_industries() -> Vec<(String, String)> {
    get_all_configs().into_iter().map(|c| (c.domain_pack_id, c.capability_pack_name)).collect()
}

// ── 共享类型（迁移自 capability_pack/mod.rs） ──
...
pub struct CapabilityPackDashboard {
    pub domain_pack_id: String,
    pub kpis: Vec<KpiValue>,
    pub cards: Vec<DashboardCard>,
    pub summary: Option<String>,
    /// 风控判定等级（复用 [`OpcRiskGate`](super::analysis::OpcRiskGate)，与
    /// `opc_execute_analysis` **同一判据、同一阈值来源**：域包 `runtime.yaml`）。
    ///
    /// `None` = **未判定**（装配时没跑门控），**不是**「无风险」；消费方不得把空值
    /// 当作 `low` 展示。
    pub risk_level: Option<super::analysis::RiskLevel>,
    /// 判定命中的违规明细（与 `risk_level` 同源；未判定 ⇒ 空）。
    pub violations: Vec<super::analysis::RiskViolation>,
}
...
mod tests {
    ...
    use axagent_harness::knowledge_graph::ENTITY_TYPE_DECLS;

    use super::*;

    /// **命名空间边界**：OPC 业务记录类型（`CapabilityPackConfig.entity_types`）与知识图谱实体类型
    /// （`knowledge_entities.entity_type` / `harness::ENTITY_TYPE_DECLS`）是**两套词表**，
    /// 字段同名、值域必须**不相交**（判据 K「一名多义」）。
    ///
    /// 为什么这条值得一条测试：
    /// ① 交集一旦非空，同一个字面量（比如 `company`）在两个层级指两种概念，任何一处
    ///    按字面量比较的代码都会静默错——这正是 `knowledge_loader` 曾经踩的坑的同类形态；
    /// ② 「项目里到底有几套实体类型词表」此前只散落在 14 个域包配置里，**无人全局可见**
    ///    （2026-09-14 复盘实测：两套词表值域完全不相交，但这一点从未被任何代码断言过）。
    ///
    /// 本测试**不复制**那张 44 条清单（复制就是第三份副本，必腐烂）——它直接读运行时的
    /// `get_all_configs()`，只断言「与 KG 词表无交集」+「规模下界」。
    #[test]
    fn test_opc_record_types_do_not_collide_with_kg_entity_types() {
        let configs = get_all_configs();
        assert!(configs.len() >= 14, "域包配置数异常（实测 14 个）：{}", configs.len());

        let mut domain_pack_ids: HashSet<&str> = HashSet::new();
        let mut all_types: Vec<(&str, &str)> = Vec::new();
        for c in &configs {
            assert!(
                domain_pack_ids.insert(&c.domain_pack_id),
                "域包 id 重复：{}",
                c.domain_pack_id
            );
            assert!(
                !c.entity_types.is_empty(),
                "域包 {} 的 entity_types 不得为空",
                c.domain_pack_id
            );
            ...
            for t in &c.entity_types {
                assert!(
                    ...
                    "域包 {} 的 entity_types 内部重复：{t}",
                    c.domain_pack_id
                );
                all_types.push((c.domain_pack_id.as_str(), t.as_str()));
            }
        }
        // 规模下界：防「被清空后测试仍然通过」的退化（实测 44 条）
        assert!(all_types.len() >= 40, "业务记录类型总数异常（实测 44）：{}", all_types.len());

        let collisions: Vec<String> = all_types
            .iter()
            .filter(|(_, t)| ENTITY_TYPE_DECLS.iter().any(|d| d.id.eq_ignore_ascii_case(t)))
            .map(|(ind, t)| format!("{ind}.entity_type = {t}"))
            .collect();
        assert!(
            collisions.is_empty(),
            "OPC 业务记录类型与知识图谱实体类型**值域相交** ⇒ 同一个字面量在两个层级指两种概念：{collisions:?}"
        );
    }
}
